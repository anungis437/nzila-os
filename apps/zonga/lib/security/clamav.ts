/**
 * ClamAV malware scanning bridge — Zonga.
 *
 * Mirror of `apps/union-eyes/lib/security/clamav.ts` so every Zonga blob
 * upload can route through `assertBufferSafeForUpload` before persistence.
 * Keep the two implementations in lock-step; if one changes the wire
 * protocol, the other must follow.
 */
...
function mapJsonResponse(payload: unknown): MalwareScanResult {
  ...
  if (status === 'unavailable') {
    return {
      ...
      engine: 'clamav',
      signature,
      reason: reason ?? 'scanner_unavailable',
    }
  }
...
export async function assertBufferSafeForUpload(
  ...
  if (result.status === 'unavailable' && shouldFailClosed()) {
    throw new MalwareScanError(`Malware scanner unavailable for ${fileLabel}`, result)
  }
