...
describe('Subscription invariant guards', () => {
  ...
  describe('S2: guardCreatorFeature', () => {
    it('grants label user access to label-only features', () => {
      expect(guardCreatorFeature('label', 'roster_management').passed).toBe(true)
      expect(guardCreatorFeature('label', 'automated_royalty_splits').passed).toBe(true)
      expect(guardCreatorFeature('label', 'advanced_analytics').passed).toBe(true)
      expect(guardCreatorFeature('label', 'bulk_upload').passed).toBe(true)
      expect(guardCreatorFeature('label', 'compliance_exports').passed).toBe(true)
    })

    it('grants artist user access to artist features', () => {
      expect(guardCreatorFeature('artist', 'upload_content').passed).toBe(true)
      expect(guardCreatorFeature('artist', 'basic_analytics').passed).toBe(true)
      expect(guardCreatorFeature('artist', 'payouts').passed).toBe(true)
    })

    it('blocks artist user from label-only features', () => {
      const result = guardCreatorFeature('artist', 'roster_management')
      expect(result.passed).toBe(false)
      expect(result.invariant).toBe('S2_CREATOR_FEATURE_ACCESS')
      expect(result.details).toContain('label')
    })

    it('blocks artist from all label-only features', () => {
      const labelFeatures = [
      ...
      for (const feature of labelFeatures) {
        expect(guardCreatorFeature('artist', feature).passed).toBe(false)
      }
    })
  ...
  describe('S3: guardCreatorFeature (enterprise)', () => {
    it('blocks label user from enterprise-only features', () => {
      const result = guardCreatorFeature('label', 'api_access')
      expect(result.passed).toBe(false)
      expect(result.invariant).toBe('S3_ENTERPRISE_FEATURE_ACCESS')
      expect(result.details).toContain('enterprise')
    })

    it('blocks artist from enterprise-only features', () => {
      const result = guardCreatorFeature('artist', 'white_label')
      expect(result.passed).toBe(false)
      expect(result.invariant).toBe('S3_ENTERPRISE_FEATURE_ACCESS')
    })
    ...
    it('blocks all enterprise-only features for non-enterprise plans', () => {
      const enterpriseFeatures = [
        'api_access', 'white_label', 'sla_guarantees',
        'on_premise', 'custom_payment_rails', 'rights_management',
      ] as const
      for (const feature of enterpriseFeatures) {
        expect(guardCreatorFeature('label', feature).passed).toBe(false)
        expect(guardCreatorFeature('artist', feature).passed).toBe(false)
      }
    })
  })
  ...
  describe('guardCreatorPlanTier', () => {
    it('passes when current plan meets required tier', () => {
      expect(guardCreatorPlanTier('label', 'label').passed).toBe(true)
      expect(guardCreatorPlanTier('enterprise', 'label').passed).toBe(true)
      expect(guardCreatorPlanTier('enterprise', 'artist').passed).toBe(true)
      expect(guardCreatorPlanTier('artist', 'artist').passed).toBe(true)
    })

    it('fails when current plan is below required tier', () => {
      const result = guardCreatorPlanTier('artist', 'label')
      expect(result.passed).toBe(false)
      expect(result.invariant).toBe('S2_PLAN_TIER_CHECK')
      expect(result.details).toContain('artist')
      expect(result.details).toContain('label')
    })

    it('fails label when enterprise required', () => {
      expect(guardCreatorPlanTier('label', 'enterprise').passed).toBe(false)
    })
  })
  ...
  describe('isCreatorLabel', () => {
    it('returns true for label + active', () => {
      expect(isCreatorLabel('label', 'active')).toBe(true)
    })
    ...
    it('returns true for label + trialing', () => {
      expect(isCreatorLabel('label', 'trialing')).toBe(true)
    })

    it('returns false for artist plan', () => {
      expect(isCreatorLabel('artist', 'active')).toBe(false)
    })

    it('returns false for label + canceled', () => {
      expect(isCreatorLabel('label', 'canceled')).toBe(false)
    })
