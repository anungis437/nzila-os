...
def _handle_user_created(data: Dict[str, Any]):
    """Create Django User when new user signs up via Clerk."""
    ...
    logger.info(f"{action}: user {clerk_user_id}")


def _handle_user_updated(data: Dict[str, Any]):
    """Sync user metadata changes from Clerk."""
    clerk_user_id = data.get("id")
    try:
...
def _handle_organization_created(data: Dict[str, Any]):
    """Create or update local org from Clerk organization.created event.
    ...
    logger.info(f"Organization created in Clerk: {clerk_org_id} — {org_name}")
...
def _handle_organization_updated(data: Dict[str, Any]):
    ...
    clerk_org_id = data.get("id")
    org_name = data.get("name", "")
    logger.info(f"Organization updated in Clerk: {clerk_org_id} — {org_name}")
...
def _handle_membership_created(data: Dict[str, Any]):
    ...
    role = _map_clerk_role(data.get("role", "member"))
    ...
    try:
        org = Organizations.objects.get(clerk_org_id=clerk_org_id)
...
def _handle_membership_deleted(data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(
            f"Deactivated {updated} membership(s) for user {clerk_user_id} in org {clerk_org_id}"
        )
    except Organizations.DoesNotExist:
