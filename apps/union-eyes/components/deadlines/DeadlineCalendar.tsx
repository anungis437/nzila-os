    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="p-6">
        <div className="grid grid-cols-[1fr_300px] gap-6">
                      ...
                      <div className="space-y-1">
                        ...
                        {dayDeadlines.length > 3 && (
                          <div className="text-xs text-gray-500 text-center">
                            {t('moreItems', { count: dayDeadlines.length - 3 })}
                          </div>
                        )}
                      </div>
                    )}
          ...
          <div className="border-l border-gray-200 pl-6">
            {selectedDate ? (
              ...
              <div className="text-center py-8">
                <ClockIcon className="mx-auto h-10 w-10 text-gray-300 mb-2" />
                <p className="text-sm text-gray-500">{t('noDeadlines')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <p className="text-xs font-medium text-gray-700 mb-3">{t('statusLegend')}</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-green-500 border border-green-600"></div>
              <span className="text-xs text-gray-600">{t('legendSafe')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-yellow-500 border border-yellow-600"></div>
              <span className="text-xs text-gray-600">{t('legendWarning')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-red-600 border border-red-700"></div>
              <span className="text-xs text-gray-600">{t('legendUrgent')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-gray-900 border border-gray-900"></div>
              <span className="text-xs text-gray-600">{t('legendOverdue')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-gray-400 border border-gray-500"></div>
              <span className="text-xs text-gray-600">{t('legendCompleted')}</span>
            </div>
          </div>
        </div>
      </div>
