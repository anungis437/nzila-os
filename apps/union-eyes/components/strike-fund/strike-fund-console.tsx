...
export default function StrikeFundConsole() {
  ...
  const fetchFunds = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch strike funds');
  ...
  const getStatusBadge = (status: string) => {
    const config = {
      ...
      active: { label: 'Active Strike', variant: 'destructive' as const },
                  ...
                  <Card key={fund.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <Link href={`${localePrefix}/dashboard/strike-fund/${fund.id}`}>
