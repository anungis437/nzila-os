...
import { IndustrialAllianceClient } from '../../../adapters/insurance/ia-client';
...
const makeClient = (over: Record<string, unknown> = {}) =>
  new IndustrialAllianceClient({ clientId: 'cid', clientSecret: 'secret', groupAccountId: 'ga1', refreshToken: 'rt', environment: 'sandbox', ...over });

describe('IndustrialAllianceClient', () => {
  beforeEach(() => {
    queue.length = 0;
  ...
  it('getPolicies returns policies', async () => {
    const client = makeClient();
    pushResp(tokenResponse());
    pushResp(resp({ policies: [{ policyId: '1' }] }));
    const r = await client.getPolicies(1, 100, new Date('2023-01-01'));
  ...
  it('getBeneficiaries returns beneficiaries', async () => {
    const client = makeClient();
    pushResp(tokenResponse());
    pushResp(resp({ beneficiaries: [{ beneficiaryId: '1' }] }));
    const r = await client.getBeneficiaries();
    expect(r).toHaveLength(1);
  });
  ...
  it('getUtilization returns utilization', async () => {
    const client = makeClient();
    pushResp(tokenResponse());
    pushResp(resp({ utilization: [{ utilizationId: '1' }] }));
    const r = await client.getUtilization();
    expect(r).toHaveLength(1);
  });
