...
export function generateExecutiveSummary(
  ...
  const openings: Record<MaturityBandId, string> = {
    personality_dependent: `This assessment describes an ${inst} where continuity depends, in a meaningful way, on the presence and memory of a small number of people. The institutional knowledge that makes your ${inst} function — its procedural history, its relational context, its governance rationale — is largely held by individuals rather than embedded in shared infrastructure. That is not a failing; it is a pattern that develops quietly over time in every institution that has grown faster than its documentation.`,

    fragmented_coordination: `This assessment describes an ${inst} in transition. Operational practices have been documented in parts, teams maintain their own procedures, and coordination works — but the institution does not yet function as a single, coherent continuity entity. What each team knows well, the institution as a whole holds imperfectly. The knowledge is distributed, the handoffs are relational, and the picture that governance bodies receive is reconstructed from fragments rather than drawn from a shared institutional record.`,

    structured_governance: `This assessment describes an ${inst} that has made meaningful investment in governance structure. Processes exist, roles are defined, decision records are maintained, and onboarding follows a documented path. The institution has moved beyond informal coordination and into structured practice — a significant accomplishment that many institutions underestimate. At the same time, this assessment surfaces a pattern common to institutions at this level: structured governance and embedded continuity intelligence are not the same thing.`,

    continuity_aware: `This assessment describes an ${inst} that treats continuity as a governance discipline. Operational knowledge is systematically captured, succession planning is active, and transitions are structured rather than improvised. The ${lead} of this ${inst} has made deliberate investments in continuity infrastructure — and those investments are visible in this profile. The question for institutions at this level is not whether continuity exists, but how deeply it has been embedded and how resilient it remains under stress.`,

    continuity_intelligence: `This assessment describes an ${inst} that has internalized continuity as an organizational capability rather than a governance discipline. Continuity thinking has moved from policy into practice, from structure into culture. The ${inst} maintains operational coherence across transitions, and its institutional memory is accessible, evidenced, and actively maintained. This briefing treats that posture with the seriousness it deserves while identifying the remaining gaps — because at this level, the gaps are subtle and the consequences of missing them are disproportionate.`,
  };
  ...
  const p2_high_burden = `The Composite Continuity Indicator for this assessment is ${composite} out of 100 — a ${compositeLabel} posture with a Continuity Burden Index of ${burdenScore}, indicating that a ${burdenScore >= 65 ? 'substantial' : 'meaningful'} portion of what keeps this ${inst} operational depends on informal human effort rather than embedded institutional systems. ${governance(govFragScore, inst)} The ${inst}'s continuity posture is not fragile because of any single failure — it is ${burdenScore >= 65 ? 'concentrated in people in a way that creates meaningful transition risk' : 'partially dependent on individual compensation in areas that merit structured attention'}.`;

  const p2_low_burden = `The Composite Continuity Indicator for this assessment is ${composite} out of 100 — a ${compositeLabel} posture with a Continuity Burden Index of ${burdenScore}, indicating a ${burdenScore < 30 ? 'well-distributed' : 'reasonably distributed'} continuity load across the ${inst}'s operational infrastructure. ${governance(govFragScore, inst)} ${icScore >= 65 ? `Institutional continuity is a relative strength, with operational knowledge demonstrably embedded beyond individual holders.` : `While individual dimensions reflect areas for development, the overall continuity architecture is more resilient than the composite score alone suggests.`}`;
  ...
  const p4Variants = [
    `This Executive Continuity Brief provides a full dimensional analysis of your ${inst}'s continuity posture, including governance entropy patterns, institutional memory and dependency analysis, modernization risk considerations, and a structured set of recommendations organized by planning horizon. It is intended as a governance document — to be read, discussed, and acted upon by the ${lead} of this ${inst}.`,
    `This brief organizes its findings across six dimensions of institutional continuity infrastructure, from governance coherence to operational memory, transition preparedness, and modernization risk. Each section draws directly from the data provided, interpreted through a continuity-native analytical framework designed for institutions, not for technology organizations. The ${lead} of this ${inst} may find it useful as a structured basis for a continuity planning conversation.`,
    `The pages that follow present a full analysis of this ${inst}'s continuity posture: dimensional scores, governance patterns, memory and dependency analysis, modernization risk, and recommendations calibrated to your specific situation. These findings should be treated as a starting point for governance dialogue, not a definitive verdict. Institutional continuity is a process, not a destination.`,
  ];
...
function governance(govFragScore: number, inst: string): string {
  if (govFragScore < 35)
    return `Governance fragility is a material concern in this profile — the consistency and coherence of governance practice across this ${inst} requires structured attention.`;
  if (govFragScore < 55)
    return `Governance practice has notable variation across this ${inst} — a pattern common in institutions that have grown faster than their governance infrastructure.`;
...
export function generateMemoryHoldersAnalysis(
  ...
  if (burdenScore >= 65) {
    p2 = `Institutions often underestimate the amount of invisible continuity labour quietly carried by experienced ${holders}. This labour rarely appears on any organizational chart. It is the work of translating institutional history for new colleagues, of contextualizing current decisions against past rationale, of maintaining relational coherence across personnel transitions. The people who carry it are, by definition, the most experienced and irreplaceable members of the ${inst}. The Continuity Burden Index for this assessment is ${burdenScore} — a level that indicates this invisible labour is substantive and that its current distribution concentrates transition risk in ways that warrant governance attention.`;
  } else if (burdenScore >= 40) {
    p2 = `The ${inst}'s continuity infrastructure distributes operational memory partially across embedded systems and partially across individuals. The Continuity Burden Index of ${burdenScore} reflects a moderate level of individual-carried continuity — a pattern that is manageable under stability but that creates meaningful pressure during simultaneous transitions. The ${lead} of this ${inst} carries awareness of this dynamic. Whether it has been formally named, documented, and addressed as a governance discipline is what this section examines.`;
  } else {
    p2 = `The Continuity Burden Index for this assessment is ${burdenScore} — a level that reflects meaningful investment in distributed continuity infrastructure. Operational memory in this ${inst} is carried more by embedded systems and documented practice than by individual compensation. This is an accomplishment worth naming: it represents deliberate governance investment in institutional resilience, and it protects the ${holders} from bearing disproportionate continuity responsibility.`;
  }
...
export function generateModernizationReview(
  ...
  const p3Variants = [
    `A continuity-aware approach to modernization for this ${inst} begins with a governance posture: technology investments are evaluated not only for operational benefit but for continuity impact. What institutional knowledge does this change depend on? What operational context must be preserved through this transition? What governance evidence is at risk of being lost, and how will it be protected? These are not technology questions — they are governance questions that happen to arise in a technology context. The ${lead} of this ${inst} is best positioned to answer them.`,
    `Continuity-safe modernization is not cautious modernization. It is informed modernization: adoption of tools and practices that are chosen by an ${inst} that understands its own context, implemented with deliberate attention to what must be preserved, and governed with the same accountability applied to any significant institutional decision. The Organizational Continuity Infrastructure framework does not oppose change; it insists that change be governed with institutional wisdom rather than operational urgency.`,
    `The path forward for this ${inst} on modernization is to treat continuity infrastructure investment as a co-investment with any significant operational change. Not as a pre-condition that must be completed before change is possible, but as a discipline that runs alongside change: capturing what is being replaced, what is being learned, and what the ${inst} will need to know about this transition five years from now. Technology with soul is technology that helps institutions remember their obligations to people.`,
  ];
...
export function generateExecutiveReflection(
  ...
  const acknowledgments: Record<MaturityBandId, string> = {
    personality_dependent: `Every institution that is operating at the Tribal Continuity level is being held together by people who are choosing to stay, choosing to carry what needs to be carried, and choosing to do the invisible labour of institutional coherence. That deserves acknowledgment. This briefing is not a critique of the people in this ${inst} — it is an argument for giving them better infrastructure so they don't have to carry it alone.`,
    fragmented_coordination: `Institutions that are growing — in complexity, in scope, in the demands placed on them — often find themselves at exactly this juncture: capable in parts, coherent in some domains, but not yet unified as a continuity entity. The people in this ${inst} are doing real work. The question this briefing asks is not whether they are doing enough, but whether the institutional architecture around them is helping or requiring more from them than it should.`,
    structured_governance: `This ${inst} has made deliberate investments in governance structure, and those investments are evident in this profile. Structure is not a small thing. It is the foundation on which every subsequent continuity investment is built. The gap between structured governance and embedded continuity intelligence is real, but it is a gap that structured governance institutions are uniquely positioned to close — because they have the foundation.`,
    continuity_aware: `This ${inst} understands continuity as a governance discipline, and that understanding is visible throughout this profile. What this briefing has tried to do is surface the remaining dimensions — the places where the discipline is not yet culture, where the infrastructure is not yet embedded, where the investment has been made but has not yet compounded. These are refinements, not failures.`,
  ...
  const p3 =
    `This briefing was prepared by Union Eyes as a contribution to the development of institutional continuity as a governance discipline. It is offered in the belief that organizations do their best work when they understand themselves — when they can see their own patterns clearly, name what they are carrying, and choose with intention how to move forward. The Organizational Continuity Infrastructure framework exists to make that clarity possible. Technology should help institutions remember their obligations to people.`;
