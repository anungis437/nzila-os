 ...
 *
 * This engine synthesizes organizational prose from dimension scores, signals,
 ...
 * - Prose varies genuinely because organizational situations differ genuinely.
...
export function generateExecutiveSummary(
  ...
  if (topInsight) {
    ...
    const catContext: Record<string, string> = {
      modernization_continuity_gap: `This pattern — modernization posture outpacing continuity preservation — is one of the most consequential quiet risks in organizational management. It tends to become visible only during transitions, when the context required to interpret change is no longer available.`,
      invisible_labour: `This form of organizational dependency is often invisible to governance bodies precisely because it works. The people who carry it rarely describe it as a risk — they simply continue to carry it.`,
      ...
      reconstruction_burden: `Reconstruction burden is the cost of organizational forgetting. It is borne quietly by whoever must reconstruct context that should have been preserved, most often during leadership transitions and external reviews.`,
      institutional_forgetting: `Organizational forgetting is not an event — it is a process. Knowledge does not disappear suddenly; it recedes gradually as the people who hold it move on and the systems that should have captured it were never built.`,
    };

    p3 = `${insightBody} ${catContext[insightCat] ?? ''}`.trim();
  } else if (opMemScore < 45) {
    p3 = `Operational memory — the collective capacity of this ${inst} to access, reference, and apply its organizational history — appears to be an area requiring attention. When operational memory is distributed primarily through individuals, transitions become exercises in reconstruction rather than in handover. What should take days takes weeks. What should require documentation requires relationships. This is the pattern this assessment observes.`;
  } else if (trScore < 40) {
...
export function generateGovernanceEntropyAnalysis(
  ...
  const entropyFramings: Partial<Record<Band, string>> = {
    ...
    4: `Governance practice in this ${inst} is mature and structured. The entropy analysis identifies no systemic fragility. The patterns this section examines are refinements — the difference between a ${inst} that governs well and one that governs with organizational memory embedded in every governance interaction.`,
  ...
  const driftPatterns: Partial<Record<Band, string>> = {
    ...
    5: `The governance posture of this ${inst} demonstrates organizational learning — practices that have been refined over time, documented, and embedded in operational reality. Remaining entropy is marginal and concentrated in specific areas that this brief's recommendations address.`,
  };
  ...
  const implications: Partial<Record<Band, string>> = {
    ...
    2: `When governance is fragmented, accountability is localized. Teams know their own practice; they do not always know the organizational practice. This creates meaningful risk in three situations: external review (auditors and regulators ask organizational questions, not team questions), leadership transition (incoming leaders inherit fragments, not a coherent picture), and strategic decision-making (the rationale for previous decisions is not reliably available to inform new ones).`,
    3: `The primary operational implication at this level is not fragility — it is optimization. A well-governed ${inst} that has not yet embedded organizational memory as a governance discipline will discover the cost of that gap during its next significant leadership transition. The knowledge exists. The infrastructure to preserve and transmit it is what requires investment.`,
    4: `For ${inst}s at this level, governance entropy is a refinement challenge, not a resilience challenge. The risk is not organizational disruption — it is the gradual accumulation of small inconsistencies that, if left unaddressed, eventually require deliberate effort to reconcile. Maintaining governance intelligence requires ongoing discipline, not just initial investment.`,
  ...
  if (observedSigs.length > 0 && govBand <= 3) {
    paras.push(
      `Among the continuity signals observed in this assessment, the following are directly relevant to governance entropy: ${observedSigs.slice(0, 3).join('; ')}. These signals represent recognizable organizational patterns — not diagnoses, but observations that the ${lead} of this ${inst} may find useful to hold in context while reading the analysis that follows.`,
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// PART 3 — Organizational Memory & Continuity Holders
// ─────────────────────────────────────────────────────────────────────────────
...
export function generateMemoryHoldersAnalysis(
  ...
  if (burdenScore >= 65) {
    ...
    p2 = `The Continuity Burden Index of ${burdenScore} reflects meaningful investment in distributed continuity infrastructure. Operational memory here is carried more by embedded systems and documented practice than by individual compensation. That is an accomplishment worth naming. It represents deliberate governance investment in organizational resilience, and it protects the ${holders} from bearing disproportionate continuity responsibility.`;
  }
  ...
  const transitionAnalysis: Partial<Record<Band, string>> = {
    ...
    2: `Transition readiness at ${trScore} indicates an ${inst} that handles routine role changes through relational continuity but has limited structured infrastructure for absorbing larger transitions. The difference between a routine personnel change and a significant leadership transition is not just scale — it is that larger transitions require the transfer of governance context, strategic rationale, and organizational relationship history, none of which travels well through informal means.`,
    ...
    5: `Transition readiness at ${trScore} reflects an ${inst} that treats transitions as governed processes rather than operational emergencies. Knowledge transfer is structured, handover is documented, and new entrants are onboarded into organizational context rather than reconstructing it from scratch.`,
  };
...
export function generateModernizationReview(
  ...
  if (modernizationInsight) {
    p2 = `${modernizationInsight.body} The specific risk pattern this assessment observes is one where the operational and governance structures appear to be more developed than the organizational memory infrastructure that gives those structures meaning. Technology can amplify this gap: systems are adopted, workflows are changed, and the organizational context required to understand why the previous approach existed — and what it was protecting — is not preserved.`;
  } else if (opMemScore < 45) {
    p2 = `The most significant continuity risk in any modernization effort for this ${inst} is operational memory loss. When systems change — whether through technology adoption, process redesign, or workflow migration — the organizational knowledge embedded in the old system does not automatically transfer to the new one. What staff understood implicitly about how things worked, what governance had learned about what didn't work, what the operational history reveals about this ${inst}'s specific context — none of this is captured by the technology transition itself. It must be deliberately preserved.`;
    ...
    p2 = `The continuity consideration most relevant to this ${inst}'s modernization profile is governance coherence across transitions. As systems, platforms, and workflows evolve, the governance rationale that informed previous technology decisions — what was tried, why certain approaches were chosen, what was learned — should be preserved and accessible to the ${lead} who will make the next round of technology decisions. Organizational technology history is a form of operational memory, and its loss is one of the more subtle forms of organizational forgetting.`;
  }

  // ── Paragraph 3: Continuity-safe path ─────────────────────────────────────

  const p3Variants = [
    `A continuity-aware approach to modernization begins with a governance posture, not a procurement plan. What organizational knowledge does this change depend on? What operational context must be preserved through it? What governance evidence is at risk of being lost, and how will it be protected? These are not technology questions. They are governance questions that happen to arise in a technology context, and the ${lead} of this ${inst} is best positioned to answer them.`,
    `Continuity-safe modernization is informed modernization. Tools are chosen by an ${inst} that understands its own context. Implementation pays deliberate attention to what must be preserved. Adoption is governed with the same accountability applied to any significant organizational decision. The Organizational Continuity Infrastructure framework does not oppose change; it asks that change be carried with organizational wisdom rather than operational urgency.`,
...
export function generateRecommendations(
  ...
  if (bandId === 'personality_dependent' || opMemScore < 35) {
    immediate.push({
      horizon: 'immediate',
      title: 'Begin Organizational Memory Documentation',
...
export function buildOnboardingSurvivabilityNarrative(
  ...
  const body =
    ...
    `Survivability is read from the institution''s own onboarding artefacts and the workflow completions that have been recorded for successor stewards. The reading is organizational. It is not a measurement of any individual.`;
...
export function buildStewardshipRedistributionEvolutionNarrative(
  ...
  const body =
    ...
    `Redistribution evolves only when carriers have consented and the reversibility window has closed without withdrawal. Refusals and withdrawals are read as organizational evidence, not as friction.`;
