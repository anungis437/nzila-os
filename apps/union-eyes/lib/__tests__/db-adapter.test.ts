...
describe('db-adapter', () => {
  ...
  describe('createOrgScopedQuery', () => {
    ...
    it('applyFilter calls eq on query with orgId', () => {
      ...
      const result = scoped.applyFilter(mockQuery as any as Parameters<typeof scoped.applyFilter>[0]);
    ...
    it('applyFilter uses custom column name', () => {
      ...
      const mockQuery = { eq: (col: string, val: string) => ({ col, val }) };
      const result = scoped.applyFilter(mockQuery as any as Parameters<typeof scoped.applyFilter>[0], 'tenant_id');
