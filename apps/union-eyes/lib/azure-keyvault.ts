...
export async function setEncryptionKeyInSession(
  ...
  try {
    ...
    if (dbClient.execute) {
      ...
      await dbClient.execute(sql`SET LOCAL app.encryption_key = ${encryptionKey}`);
    } else if (dbClient.query) {
      // postgres client
      await dbClient.query(`SET LOCAL app.encryption_key = $1`, [encryptionKey]);
    } else {
