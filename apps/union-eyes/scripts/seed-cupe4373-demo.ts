  ...
  await db.execute(sql`
    ...
    VALUES
      ...
      (${ENTITY_EMPLOYER_ID}::uuid,
        ${'Sioux Lookout Meno Ya Win Health Centre'}::text,
       ${'CA-ON'}::varchar,
       'active'::entity_status,
       now(), now())
...
async function seedOrganization() {
  ...
  await db.execute(sql`
    ...
    VALUES (
      ...
      ${'CUPE Local 4373'}::text,
      ${'CUPE 4373'}::text,
      ${'Healthcare local demo focused on casework continuity, grievance handling, member communication, and leadership visibility for Sioux Lookout Meno Ya Win Health Centre.'}::text,
  ...
  for (const c of demoCases) {
    ...
    await db.execute(sql`
      ...
      VALUES (
        ...
        ${ENTITY_EMPLOYER_ID}::uuid,
        ${'Sioux Lookout Meno Ya Win Health Centre'}::varchar,
        ${c.location}::varchar,
