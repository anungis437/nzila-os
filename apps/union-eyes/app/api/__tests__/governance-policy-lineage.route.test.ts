...
async function loadRoute() {
  return import('../governance/lifecycle/policies/[id]/lineage/route');
}

describe('governance/lifecycle/policies/[id]/lineage route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    m.db.select
      ...
      .mockImplementationOnce(() => ({ from: vi.fn(() => ({ where: vi.fn(() => ({ orderBy: vi.fn(async () => [{ id: 'p1', semver: '1.0.0', name: 'Policy 1', lifecycleStatus: 'draft', riskClassification: 'medium', domain: 'finance', supersededBy: null }]) })) })) }));
  });

  it('returns lineage nodes and edges', async () => {
    const { GET } = await loadRoute();
    const result = await GET({ params: { id: 'p1' } });

    expect(result.policyFamilyId).toBe('fam_1');
