...
describe('calendar-sync/microsoft/callback route', () => {
  ...
  it('returns auth required when user id is missing', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/calendar-sync/microsoft/callback?code=abc'), { organizationId: 'org_1' });
  ...
  it('returns forbidden when organization id is missing', async () => {
    const { GET } = await loadRoute();
    const response = await GET(new NextRequest('http://localhost/api/calendar-sync/microsoft/callback?code=abc'), { userId: 'u1' });
  ...
  it('returns validation error when OAuth code is missing', async () => {
    const { GET } = await loadRoute();
    const response = await GET(new NextRequest('http://localhost/api/calendar-sync/microsoft/callback'));
    expect(response.status).toBe(400);
  });

  it('returns forbidden when OAuth state mismatches user', async () => {
    const { GET } = await loadRoute();
    const response = await GET(new NextRequest('http://localhost/api/calendar-sync/microsoft/callback?code=abc&state=u999'));
    expect(response.status).toBe(403);
  });
  ...
  it('returns service unavailable when no refresh token exists for new connection', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/calendar-sync/microsoft/callback?code=oauth_code&state=u1'));

    expect(response.status).toBe(503);
  });

  it('creates a new microsoft connection and redirects', async () => {
    const { GET } = await loadRoute();
    m.selectQueue.push([]);
    ...
    expect(response.headers.get('location')).toContain('/calendar?sync=connected&provider=microsoft');
    expect(m.db.insert).toHaveBeenCalled();
  });

  it('updates existing microsoft connection when record exists', async () => {
    const { GET } = await loadRoute();
    m.selectQueue.push([{ id: 'existing_msft_conn' }]);
    m.exchangeCodeForTokens.mockResolvedValueOnce({
      accessToken: 'access_123',
      refreshToken: null,
      expiresAt: new Date(Date.now() + 3600000),
