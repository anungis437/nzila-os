...
export const PATCH = withApi(
  ...
  async ({ params, body }) => {
    ...
    const [row] = await withRLSContext(async () =>
      db.update(boardPackets).set({ ...parsed, updatedAt: new Date() }).where(eq(boardPackets.id, params.id)).returning()
    );
    if (!row) throw ApiError.notFound('Board packet not found');
    return row;
  },
...
export const DELETE = withApi(
  ...
  async ({ params }) => {
    const [row] = await withRLSContext(async () =>
      db.update(boardPackets).set({ status: 'archived', updatedAt: new Date() }).where(eq(boardPackets.id, params.id)).returning()
    );
    if (!row) throw ApiError.notFound('Board packet not found');
    return row;
  },
