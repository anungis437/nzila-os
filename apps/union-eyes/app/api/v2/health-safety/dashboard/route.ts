/**
 * GET /api/v2/health-safety/dashboard
 * Dashboard aggregation — same shape as v1 but under v2 path.
 */
...
export const GET = withApi(
  {
    ...
    openapi: {
      tags: ['Health-safety'],
      summary: 'Health & Safety dashboard metrics (v2)',
    },
  },
  async ({ request }) => {
    ...
    return withRLSContext(async (db) => {
      const incidentRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM workplace_incidents WHERE ${orgFilter} AND ${dateFilter}`));
      ...
      const prevRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM workplace_incidents WHERE ${orgFilter} AND created_at >= ${prevStart} AND created_at < ${startDate}`));
      const prevIncidents = Number((prevRows[0] as Record<string, unknown>)?.cnt ?? 0);
      const incidentChange = prevIncidents > 0 ? Math.round(((totalIncidents - prevIncidents) / prevIncidents) * 100) : 0;
      const incidentTrend = incidentChange > 0 ? 'up' : incidentChange < 0 ? 'down' : 'stable';

      const hazardRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM hazard_reports WHERE ${orgFilter} AND (status IS NULL OR status NOT IN ('resolved','closed'))`));
      ...
      const inspDueRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM safety_inspections WHERE ${orgFilter} AND ${dateFilter}`));
      const inspectionsDue = Number((inspDueRows[0] as Record<string, unknown>)?.cnt ?? 0);

      const inspCompRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM safety_inspections WHERE ${orgFilter} AND status = 'completed' AND ${dateFilter}`));
      const inspectionsCompleted = Number((inspCompRows[0] as Record<string, unknown>)?.cnt ?? 0);
      const inspectionComplianceRate = inspectionsDue > 0 ? Math.round((inspectionsCompleted / inspectionsDue) * 100) : 100;

      const lastRows = Array.from(await db.execute(sql`SELECT max(created_at)::text AS last_dt FROM workplace_incidents WHERE ${orgFilter}`));
      const lastDt = (lastRows[0] as Record<string, unknown>)?.last_dt as string | null;
      const daysWithoutIncident = lastDt ? Math.max(0, Math.floor((Date.now() - new Date(lastDt).getTime()) / 86_400_000)) : 90;

      const critRows = Array.from(await db.execute(sql`SELECT count(*)::int AS cnt FROM workplace_incidents WHERE ${orgFilter} AND severity = 'critical' AND status != 'closed'`));
