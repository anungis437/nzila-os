 ...
 * 
 * GET /api/admin/clc/analytics/multi-year-trends?years=5
 */
  ...
  try {
    ...
    if (![3, 5, 10].includes(years)) {
      logApiAuditEvent({
        ...
        userId,
        endpoint: '/api/admin/clc/analytics/multi-year-trends',
        method: 'GET',
    ...
    logApiAuditEvent({
      ...
      eventType: 'success',
      severity: 'low',
      details: { dataType: 'MULTI_YEAR_TRENDS', years },
    });
    ...
    return standardErrorResponse(
      ...
      'Failed to fetch multi-year trends',
      error
    );
  }
});
