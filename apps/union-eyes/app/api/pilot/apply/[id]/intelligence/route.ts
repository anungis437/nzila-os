...
export const POST = withApiAuth(async (request: NextRequest, context?: { params?: Promise<{ id: string }> | { id: string } }) => {
  try {
    ...
    // PR #752 round 25: this write only ever touches `pilotIntelligence` —
    // withLockedPilotMutation locks the row, re-authorizes ownership under
    // that lock (closing the rebind TOCTOU), hands back the FRESH locked
    // `application` (never a stale pre-lock snapshot), and merges only the
    // `pilotIntelligence` key back — a concurrent commercial-transition
    // write to `commercialState`/scoring/artifact history can therefore
    // never be reverted by this route, and this route can never revert one
    // committed just before it acquired the lock.
    const outcome = await withLockedPilotMutation(id, 'steward', async ({ application }) => {
      const responses = (application.responses ?? {}) as Record<string, unknown>;
      const intelligence =
        typeof responses.pilotIntelligence === 'object' && responses.pilotIntelligence
      ...
      return {
        responsesPatch: { pilotIntelligence: intelligence },
        data: { id: application.id, pilotIntelligence: intelligence },
      };
    });
