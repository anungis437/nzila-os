...
/* ─── helpers ─── */
function statusBadge(status: string, t: Awaited<ReturnType<typeof getTranslations>>) {
  switch (status) {
    case 'completed':
      return <Badge variant="default" className="bg-green-600"><CheckCircle2 className="h-3 w-3 mr-1" />{t('status.completed')}</Badge>;
    case 'failed':
      return <Badge variant="destructive"><XCircle className="h-3 w-3 mr-1" />{t('status.failed')}</Badge>;
    case 'running':
      return <Badge variant="secondary"><RefreshCw className="h-3 w-3 mr-1 animate-spin" />{t('status.running')}</Badge>;
    default:
      return <Badge variant="outline">{status}</Badge>;
  }
}

function timeAgo(dateStr: string | null, t: Awaited<ReturnType<typeof getTranslations>>): string {
...
export default async function DataSourcePage() {
  await requireUser();
  const t = await getTranslations('dataSourcePage');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t('totalSources')}</CardDescription>
            <CardTitle className="text-3xl">{totalSources}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {t('internalExternalBreakdown', {
                internalCount: internalSources.length,
                externalCount: EXTERNAL_SOURCES.length,
              })}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t('internalRecords')}</CardDescription>
            <CardTitle className="text-3xl">{formatNumber(totalInternalRecords, locale)}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {t('acrossTables', { count: internalSources.length })}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t('externalRecords')}</CardDescription>
            <CardTitle className="text-3xl">{formatNumber(totalExternalRecords, locale)}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {t('benchmarksHint')}
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>{t('syncHistory')}</CardDescription>
            <CardTitle className="text-3xl">{syncLogs.length}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {t('recentSyncOperations')}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="internal" className="space-y-6">
        <TabsList>
          <TabsTrigger value="internal" className="gap-2">
            <Table2 className="h-4 w-4" /> {t('tabInternal', { count: internalSources.length })}
          </TabsTrigger>
          <TabsTrigger value="external" className="gap-2">
            <Globe className="h-4 w-4" /> {t('tabExternal', { count: EXTERNAL_SOURCES.length })}
          </TabsTrigger>
          <TabsTrigger value="sync" className="gap-2">
            <RefreshCw className="h-4 w-4" /> {t('tabSyncLog')}
          </TabsTrigger>
        </TabsList>
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline" className="text-sm font-mono">
                        {t('rowsCount', { count: formatNumber(count, locale) })}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      <p className="text-xs text-muted-foreground font-medium">
                        {t('fieldsAndJoinables', {
                          fields: ds.fields.length,
                          joinables: ds.joinable?.length ?? 0,
                        })}
                      </p>
                      <div className="flex flex-wrap gap-1">
                        ...
                        {ds.fields.length > 6 && (
                          <Badge variant="secondary" className="text-[10px]">
                            {t('moreFields', { count: ds.fields.length - 6 })}
                          </Badge>
                        )}
                      </div>
                      {ds.joinable && ds.joinable.length > 0 && (
                        <p className="text-[10px] text-muted-foreground">
                          {t('joinsPrefix')}: {ds.joinable.join(', ')}
                        </p>
                      )}
                    </div>
                  </CardContent>
              ...
              return (
                <Card key={src.id}>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-base">{t(`externalSources.${src.id}.name`)}</CardTitle>
                          <CardDescription className="text-xs">{t(`externalSources.${src.id}.provider`)}</CardDescription>
                        </div>
                      </div>
                      <Badge variant="outline" className="text-sm font-mono">
                        {t('rowsCount', { count: formatNumber(count, locale) })}
                  ...
                  <CardContent className="space-y-3">
                    <p className="text-sm text-muted-foreground">{t(`externalSources.${src.id}.description`)}</p>
                    <div className="flex items-center justify-between text-xs">
                      <span className="flex items-center gap-1 text-muted-foreground">
                        <Clock className="h-3 w-3" /> {t('scheduleLabel')}: {t(`externalSources.${src.id}.schedule`)}
                      </span>
                    ...
                    {latestSync && (
                      <div className="flex items-center justify-between text-xs border-t pt-2">
                        <span className="text-muted-foreground">
                          {t('lastSyncLabel')}: {timeAgo(latestSync.started_at, t)}
                        </span>
                        {statusBadge(latestSync.status, t)}
                      </div>
                    )}
                    {!latestSync && (
          ...
          {syncLogs.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-muted-foreground">{t('noSyncOperationsYet')}</p>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">{t('recentSyncOperationsTitle')}</CardTitle>
                <CardDescription>{t('lastSyncRunsDescription', { count: syncLogs.length })}</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-left text-xs text-muted-foreground">
                        <th className="pb-2 pr-4">{t('tableSource')}</th>
                        <th className="pb-2 pr-4">{t('tableType')}</th>
                        <th className="pb-2 pr-4">{t('tableStatus')}</th>
                        <th className="pb-2 pr-4 text-right">{t('tableProcessed')}</th>
                        <th className="pb-2 pr-4 text-right">{t('tableInserted')}</th>
                        <th className="pb-2 pr-4 text-right">{t('tableFailed')}</th>
                        <th className="pb-2">{t('tableStarted')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {syncLogs.map((log) => (
                        <tr key={log.id} className="border-b last:border-0 hover:bg-muted/50">
                          ...
                          <td className="py-2 pr-4">{statusBadge(log.status, t)}</td>
                          <td className="py-2 pr-4 text-right font-mono">{formatNumber(log.records_processed, locale)}</td>
                          <td className="py-2 pr-4 text-right font-mono">{formatNumber(log.records_inserted, locale)}</td>
                          <td className="py-2 pr-4 text-right font-mono">
                            {log.records_failed > 0 ? (
                              <span className="text-destructive">{formatNumber(log.records_failed, locale)}</span>
                            ) : (
                              formatNumber(0, locale)
                            )}
                          </td>
                          <td className="py-2 text-muted-foreground">{timeAgo(log.started_at, t)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
