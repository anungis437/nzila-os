...
/**
 * Agreements Page — Client Component
      ...
      try {
        ...
        if (res.ok) {
          ...
          const items: Agreement[] = list.map((r: any) => ({
            ...
            type: r.industrySector?.includes('letter') ? 'side-letter'
              : r.industrySector?.includes('mou') ? 'mou'
              : r.industrySector?.includes('policy') ? 'policy'
              : r.industrySector?.includes('amendment') ? 'amendment'
              : 'collective-bargaining',
            ...
            description: r.bargainingUnitDescription ?? `${r.unionName ?? ''} – ${r.employerName ?? ''}`,
            fileSize: '',
            pageCount: 0,
            lastUpdated: r.updatedAt ?? '',
            version: r.version ? String(r.version) : '1.0',
            keyTerms: Array.isArray(r.keyTerms) ? r.keyTerms : [],
            summary: r.summaryGenerated ?? '',
          }));
          setAgreements(items);
        }
      } catch {
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      >
        ...
        <div className="space-y-6">
          {loading ? (
            <div className="text-center py-12 text-muted-foreground">Loading agreements...</div>
          ) : (
            <AnimatePresence>
