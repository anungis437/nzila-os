...
DO $$ BEGIN
  IF to_regclass('ai_clause_reasonings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_clause_reasonings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ai_clause_reasonings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_clause_reasonings', 'ue_create_direct_org_rls_policy', 'ai_clause_reasonings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ai_copilot_sessions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_copilot_sessions' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('ai_copilot_sessions', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_copilot_sessions', 'ue_create_user_rls_policy', 'ai_copilot_sessions.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ai_insight_reports') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_insight_reports' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ai_insight_reports', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_insight_reports', 'ue_create_direct_org_rls_policy', 'ai_insight_reports.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ai_usage_metrics') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_usage_metrics' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ai_usage_metrics', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_usage_metrics', 'ue_create_direct_org_rls_policy', 'ai_usage_metrics.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('analytics_metrics') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'analytics_metrics' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('analytics_metrics', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'analytics_metrics', 'ue_create_direct_org_rls_policy', 'analytics_metrics.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('customer_nps_surveys') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('employer_risk_scores') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_risk_scores' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_risk_scores', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_risk_scores', 'ue_create_direct_org_rls_policy', 'employer_risk_scores.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('icra_continuity_scores') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('icra_followup_recommendations') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('insight_recommendations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'insight_recommendations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('insight_recommendations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'insight_recommendations', 'ue_create_direct_org_rls_policy', 'insight_recommendations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ml_predictions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ml_predictions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ml_predictions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ml_predictions', 'ue_create_direct_org_rls_policy', 'ml_predictions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('model_metadata') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'model_metadata' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('model_metadata', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'model_metadata', 'ue_create_direct_org_rls_policy', 'model_metadata.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_metrics') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_metrics' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_metrics', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_metrics', 'ue_create_direct_org_rls_policy', 'pilot_metrics.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('social_analytics') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'social_analytics' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('social_analytics', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'social_analytics', 'ue_create_direct_org_rls_policy', 'social_analytics.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('claim_deadlines') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'claim_deadlines' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('claim_deadlines', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'claim_deadlines', 'ue_create_direct_org_rls_policy', 'claim_deadlines.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('deadline_reminders') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'deadline_reminders' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('deadline_reminders', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'deadline_reminders', 'ue_create_direct_org_rls_policy', 'deadline_reminders.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_case_access_assignments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_case_access_assignments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('grievance_case_access_assignments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_case_access_assignments', 'ue_create_direct_org_rls_policy', 'grievance_case_access_assignments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ai_grievance_triages') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_grievance_triages' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ai_grievance_triages', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_grievance_triages', 'ue_create_direct_org_rls_policy', 'ai_grievance_triages.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('arbitrations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'arbitrations' AND column_name = 'grievance_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievances' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('arbitrations', 'grievance_id', 'grievances', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'arbitrations', 'ue_create_parent_owned_rls_policy_v2', 'arbitrations.grievance_id, grievances.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bargaining_notes') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bargaining_notes' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('bargaining_notes', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bargaining_notes', 'ue_create_direct_org_rls_policy', 'bargaining_notes.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bargaining_units') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bargaining_units' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('bargaining_units', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bargaining_units', 'ue_create_direct_org_rls_policy', 'bargaining_units.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('case_documents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'case_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('case_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'case_documents', 'ue_create_direct_org_rls_policy', 'case_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cba_clauses') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cba_clauses' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cba_clauses', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cba_clauses', 'ue_create_direct_org_rls_policy', 'cba_clauses.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cba_rule_set_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cba_rule_set_items' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cba_rule_set_items', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cba_rule_set_items', 'ue_create_direct_org_rls_policy', 'cba_rule_set_items.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cba_rule_versions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cba_rule_versions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cba_rule_versions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cba_rule_versions', 'ue_create_direct_org_rls_policy', 'cba_rule_versions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('collective_agreements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'collective_agreements' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('collective_agreements', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'collective_agreements', 'ue_create_direct_org_rls_policy', 'collective_agreements.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_insurance_claims') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_insurance_claims' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_insurance_claims', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_insurance_claims', 'ue_create_direct_org_rls_policy', 'external_insurance_claims.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_documents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('grievance_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_documents', 'ue_create_direct_org_rls_policy', 'grievance_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_settlements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_settlements' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('grievance_settlements', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_settlements', 'ue_create_direct_org_rls_policy', 'grievance_settlements.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_timeline_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_timeline_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('grievance_timeline_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_timeline_events', 'ue_create_direct_org_rls_policy', 'grievance_timeline_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_transitions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_transitions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('grievance_transitions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_transitions', 'ue_create_direct_org_rls_policy', 'grievance_transitions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_benefit_claims') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_benefit_claims' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_benefit_claims', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_benefit_claims', 'ue_create_direct_org_rls_policy', 'pension_benefit_claims.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  IF to_regclass('wcb_claims') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'wcb_claims' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('wcb_claims', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'wcb_claims', 'ue_create_direct_org_rls_policy', 'wcb_claims.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('arbitration_precedents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'arbitration_precedents' AND column_name = 'source_organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('arbitration_precedents', 'source_organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'arbitration_precedents', 'ue_create_direct_org_rls_policy', 'arbitration_precedents.source_organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bargaining_proposals') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bargaining_proposals' AND column_name = 'negotiation_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'negotiations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('bargaining_proposals', 'negotiation_id', 'negotiations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bargaining_proposals', 'ue_create_parent_owned_rls_policy_v2', 'bargaining_proposals.negotiation_id, negotiations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('claim_updates') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'claim_updates' AND column_name = 'claim_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'claims' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('claim_updates', 'claim_id', 'claims', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'claim_updates', 'ue_create_parent_owned_rls_policy_v2', 'claim_updates.claim_id, claims.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_events' AND column_name = 'grievance_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievances' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('grievance_events', 'grievance_id', 'grievances', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_events', 'ue_create_parent_owned_rls_policy_v2', 'grievance_events.grievance_id, grievances.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('grievance_timeline') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievance_timeline' AND column_name = 'grievance_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'grievances' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('grievance_timeline', 'grievance_id', 'grievances', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'grievance_timeline', 'ue_create_parent_owned_rls_policy_v2', 'grievance_timeline.grievance_id, grievances.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'campaigns', 'ue_create_direct_org_rls_policy', 'campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('chat_sessions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chat_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('chat_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'chat_sessions', 'ue_create_direct_org_rls_policy', 'chat_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('communication_preferences') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'communication_preferences' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('communication_preferences', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'communication_preferences', 'ue_create_direct_org_rls_policy', 'communication_preferences.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('consent_records') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'consent_records' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('consent_records', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'consent_records', 'ue_create_direct_org_rls_policy', 'consent_records.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cookie_consents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cookie_consents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cookie_consents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cookie_consents', 'ue_create_direct_org_rls_policy', 'cookie_consents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('data_aggregation_consent') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'data_aggregation_consent' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('data_aggregation_consent', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'data_aggregation_consent', 'ue_create_direct_org_rls_policy', 'data_aggregation_consent.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('donation_campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'donation_campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('donation_campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'donation_campaigns', 'ue_create_direct_org_rls_policy', 'donation_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_communications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_communications' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_communications', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_communications', 'ue_create_direct_org_rls_policy', 'employer_communications.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_communication_channels') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_communication_channels' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_communication_channels', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_communication_channels', 'ue_create_direct_org_rls_policy', 'external_communication_channels.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_communication_messages') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_communication_messages' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_communication_messages', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_communication_messages', 'ue_create_direct_org_rls_policy', 'external_communication_messages.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_communication_users') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_communication_users' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_communication_users', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_communication_users', 'ue_create_direct_org_rls_policy', 'external_communication_users.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('in_app_notifications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'in_app_notifications' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('in_app_notifications', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'in_app_notifications', 'ue_create_direct_org_rls_policy', 'in_app_notifications.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('message_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'message_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('message_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'message_log', 'ue_create_direct_org_rls_policy', 'message_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('newsletter_campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('newsletter_campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'newsletter_campaigns', 'ue_create_direct_org_rls_policy', 'newsletter_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('newsletter_distribution_lists') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_distribution_lists' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('newsletter_distribution_lists', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'newsletter_distribution_lists', 'ue_create_direct_org_rls_policy', 'newsletter_distribution_lists.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('notification_delivery_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'notification_delivery_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('notification_delivery_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'notification_delivery_log', 'ue_create_direct_org_rls_policy', 'notification_delivery_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('notification_queue') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'notification_queue' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('notification_queue', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'notification_queue', 'ue_create_direct_org_rls_policy', 'notification_queue.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('notification_tracking') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'notification_tracking' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('notification_tracking', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'notification_tracking', 'ue_create_direct_org_rls_policy', 'notification_tracking.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('notifications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'notifications' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('notifications', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'notifications', 'ue_create_direct_org_rls_policy', 'notifications.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('organizing_campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'organizing_campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('organizing_campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'organizing_campaigns', 'ue_create_direct_org_rls_policy', 'organizing_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('push_notifications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'push_notifications' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('push_notifications', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'push_notifications', 'ue_create_direct_org_rls_policy', 'push_notifications.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('sms_campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'sms_campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('sms_campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'sms_campaigns', 'ue_create_direct_org_rls_policy', 'sms_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('sms_conversations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'sms_conversations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('sms_conversations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'sms_conversations', 'ue_create_direct_org_rls_policy', 'sms_conversations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('sms_messages') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'sms_messages' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('sms_messages', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'sms_messages', 'ue_create_direct_org_rls_policy', 'sms_messages.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('social_campaigns') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'social_campaigns' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('social_campaigns', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'social_campaigns', 'ue_create_direct_org_rls_policy', 'social_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('user_consents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'user_consents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('user_consents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'user_consents', 'ue_create_direct_org_rls_policy', 'user_consents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('user_notification_preferences') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'user_notification_preferences' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('user_notification_preferences', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'user_notification_preferences', 'ue_create_user_rls_policy', 'user_notification_preferences.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('chat_messages') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chat_messages' AND column_name = 'session_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chat_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('chat_messages', 'session_id', 'chat_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'chat_messages', 'ue_create_parent_owned_rls_policy_v2', 'chat_messages.session_id, chat_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('newsletter_list_subscribers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_list_subscribers' AND column_name = 'list_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_distribution_lists' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'newsletter_list_subscribers', 'ue_create_parent_owned_rls_policy_v2', 'newsletter_list_subscribers.list_id, newsletter_distribution_lists.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('newsletter_recipients') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_recipients' AND column_name = 'campaign_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'newsletter_campaigns' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'newsletter_recipients', 'ue_create_parent_owned_rls_policy_v2', 'newsletter_recipients.campaign_id, newsletter_campaigns.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('provincial_consent') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'provincial_consent' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('provincial_consent', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'provincial_consent', 'ue_create_user_rls_policy', 'provincial_consent.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cms_media_library') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cms_media_library' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cms_media_library', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cms_media_library', 'ue_create_direct_org_rls_policy', 'cms_media_library.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('document_access_grants') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'document_access_grants' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('document_access_grants', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'document_access_grants', 'ue_create_direct_org_rls_policy', 'document_access_grants.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('document_folders') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'document_folders' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('document_folders', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'document_folders', 'ue_create_direct_org_rls_policy', 'document_folders.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('document_links') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'document_links' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('document_links', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'document_links', 'ue_create_direct_org_rls_policy', 'document_links.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('document_versions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'document_versions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('document_versions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'document_versions', 'ue_create_direct_org_rls_policy', 'document_versions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_execution_evidence_links') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_execution_evidence_links' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_execution_evidence_links', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_execution_evidence_links', 'ue_create_direct_org_rls_policy', 'employer_execution_evidence_links.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_execution_profiles') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_execution_profiles' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_execution_profiles', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_execution_profiles', 'ue_create_direct_org_rls_policy', 'employer_execution_profiles.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('exit_interview_documents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'exit_interview_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('exit_interview_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'exit_interview_documents', 'ue_create_direct_org_rls_policy', 'exit_interview_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_communication_files') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_communication_files' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_communication_files', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_communication_files', 'ue_create_direct_org_rls_policy', 'external_communication_files.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('icra_maturity_profiles') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('message_templates') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'message_templates' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('message_templates', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'message_templates', 'ue_create_direct_org_rls_policy', 'message_templates.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('notification_templates') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'notification_templates' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('notification_templates', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'notification_templates', 'ue_create_direct_org_rls_policy', 'notification_templates.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('signature_documents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'signature_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('signature_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'signature_documents', 'ue_create_direct_org_rls_policy', 'signature_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('document_signers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'document_signers' AND column_name = 'document_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'signature_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('document_signers', 'document_id', 'signature_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'document_signers', 'ue_create_parent_owned_rls_policy_v2', 'document_signers.document_id, signature_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('profiles') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'profiles' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('profiles', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'profiles', 'ue_create_user_rls_policy', 'profiles.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('account_mappings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'account_mappings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_mixed_global_tenant_rls_policy('account_mappings', 'organization_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'account_mappings', 'ue_create_mixed_global_tenant_rls_policy', 'account_mappings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bank_accounts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bank_accounts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('bank_accounts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bank_accounts', 'ue_create_direct_org_rls_policy', 'bank_accounts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bank_reconciliation') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bank_reconciliation' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('bank_reconciliation', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bank_reconciliation', 'ue_create_direct_org_rls_policy', 'bank_reconciliation.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('bank_reconciliations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'bank_reconciliations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('bank_reconciliations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'bank_reconciliations', 'ue_create_direct_org_rls_policy', 'bank_reconciliations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('billing_accounts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'billing_accounts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('billing_accounts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'billing_accounts', 'ue_create_direct_org_rls_policy', 'billing_accounts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('billing_periods') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'billing_periods' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('billing_periods', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'billing_periods', 'ue_create_direct_org_rls_policy', 'billing_periods.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('billing_subscriptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'billing_subscriptions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('billing_subscriptions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'billing_subscriptions', 'ue_create_direct_org_rls_policy', 'billing_subscriptions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('budget_pool') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'budget_pool' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('budget_pool', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'budget_pool', 'ue_create_direct_org_rls_policy', 'budget_pool.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('chart_of_accounts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chart_of_accounts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('chart_of_accounts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'chart_of_accounts', 'ue_create_direct_org_rls_policy', 'chart_of_accounts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('clc_remittance_mapping') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'clc_remittance_mapping' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('clc_remittance_mapping', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'clc_remittance_mapping', 'ue_create_direct_org_rls_policy', 'clc_remittance_mapping.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cost_centers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cost_centers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cost_centers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cost_centers', 'ue_create_direct_org_rls_policy', 'cost_centers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dues_assignments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dues_assignments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('dues_assignments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dues_assignments', 'ue_create_direct_org_rls_policy', 'dues_assignments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dues_rates') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dues_rates' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('dues_rates', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dues_rates', 'ue_create_direct_org_rls_policy', 'dues_rates.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dues_transactions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dues_transactions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('dues_transactions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dues_transactions', 'ue_create_direct_org_rls_policy', 'dues_transactions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_payroll_run_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_payroll_run_items' AND column_name = 'payroll_run_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_payroll_runs' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_payroll_run_items', 'ue_create_parent_owned_rls_policy_v2', 'employer_payroll_run_items.payroll_run_id, employer_payroll_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_payroll_runs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_payroll_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_payroll_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_payroll_runs', 'ue_create_direct_org_rls_policy', 'employer_payroll_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_remittance_run_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_remittance_run_items' AND column_name = 'remittance_run_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_remittance_runs' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_remittance_run_items', 'ue_create_parent_owned_rls_policy_v2', 'employer_remittance_run_items.remittance_run_id, employer_remittance_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_remittance_runs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_remittance_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_remittance_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_remittance_runs', 'ue_create_direct_org_rls_policy', 'employer_remittance_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_remittances') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_remittances' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_remittances', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_remittances', 'ue_create_direct_org_rls_policy', 'employer_remittances.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('entitlement_usage_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'entitlement_usage_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('entitlement_usage_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'entitlement_usage_log', 'ue_create_direct_org_rls_policy', 'entitlement_usage_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('erp_invoices') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'erp_invoices' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('erp_invoices', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'erp_invoices', 'ue_create_direct_org_rls_policy', 'erp_invoices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('financial_periods') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'financial_periods' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('financial_periods', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'financial_periods', 'ue_create_direct_org_rls_policy', 'financial_periods.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('gl_account_mappings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'gl_account_mappings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('gl_account_mappings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'gl_account_mappings', 'ue_create_direct_org_rls_policy', 'gl_account_mappings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('gl_transaction_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'gl_transaction_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('gl_transaction_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'gl_transaction_log', 'ue_create_direct_org_rls_policy', 'gl_transaction_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('gl_trial_balance') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'gl_trial_balance' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('gl_trial_balance', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'gl_trial_balance', 'ue_create_direct_org_rls_policy', 'gl_trial_balance.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payment_cycles') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payment_cycles' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payment_cycles', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payment_cycles', 'ue_create_direct_org_rls_policy', 'payment_cycles.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payment_disputes') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payment_disputes' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payment_disputes', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payment_disputes', 'ue_create_direct_org_rls_policy', 'payment_disputes.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payment_methods') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payment_methods' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payment_methods', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payment_methods', 'ue_create_direct_org_rls_policy', 'payment_methods.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payment_plans') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payment_plans' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payment_plans', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payment_plans', 'ue_create_direct_org_rls_policy', 'payment_plans.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payments', 'ue_create_direct_org_rls_policy', 'payments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payroll_deductions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payroll_deductions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('payroll_deductions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payroll_deductions', 'ue_create_direct_org_rls_policy', 'payroll_deductions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_contributions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_contributions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_contributions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_contributions', 'ue_create_direct_org_rls_policy', 'pension_contributions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_plans') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_plans' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_plans', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_plans', 'ue_create_direct_org_rls_policy', 'pension_plans.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_t4a_records') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_t4a_records' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_t4a_records', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_t4a_records', 'ue_create_direct_org_rls_policy', 'pension_t4a_records.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_trustee_meetings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_trustee_meetings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_trustee_meetings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_trustee_meetings', 'ue_create_direct_org_rls_policy', 'pension_trustee_meetings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pension_trustees') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_trustees' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_trustees', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_trustees', 'ue_create_direct_org_rls_policy', 'pension_trustees.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('per_capita_remittances') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'per_capita_remittances' AND column_name = 'from_organization_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'per_capita_remittances' AND column_name = 'to_organization_id') THEN
      PERFORM ue_create_multi_party_rls_policy('per_capita_remittances', 'from_organization_id', 'to_organization_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'per_capita_remittances', 'ue_create_multi_party_rls_policy', 'per_capita_remittances.from_organization_id, per_capita_remittances.to_organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('platform_cost_ledger_entries') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_cost_ledger_entries' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('platform_cost_ledger_entries', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'platform_cost_ledger_entries', 'ue_create_direct_org_rls_policy', 'platform_cost_ledger_entries.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('platform_invoices') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_invoices' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('platform_invoices', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'platform_invoices', 'ue_create_direct_org_rls_policy', 'platform_invoices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('platform_payments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_payments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('platform_payments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'platform_payments', 'ue_create_direct_org_rls_policy', 'platform_payments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reconciliation_exceptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reconciliation_exceptions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reconciliation_exceptions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reconciliation_exceptions', 'ue_create_direct_org_rls_policy', 'reconciliation_exceptions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reconciliation_runs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reconciliation_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reconciliation_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reconciliation_runs', 'ue_create_direct_org_rls_policy', 'reconciliation_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('remittance_exceptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'remittance_exceptions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('remittance_exceptions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'remittance_exceptions', 'ue_create_direct_org_rls_policy', 'remittance_exceptions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('remittance_line_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'remittance_line_items' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('remittance_line_items', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'remittance_line_items', 'ue_create_direct_org_rls_policy', 'remittance_line_items.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reward_budget_envelopes') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reward_budget_envelopes' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reward_budget_envelopes', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reward_budget_envelopes', 'ue_create_direct_org_rls_policy', 'reward_budget_envelopes.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reward_wallet_ledger') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reward_wallet_ledger' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reward_wallet_ledger', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reward_wallet_ledger', 'ue_create_direct_org_rls_policy', 'reward_wallet_ledger.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('social_accounts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'social_accounts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('social_accounts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'social_accounts', 'ue_create_direct_org_rls_policy', 'social_accounts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('subscription_events_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'subscription_events_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('subscription_events_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'subscription_events_log', 'ue_create_direct_org_rls_policy', 'subscription_events_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('payment_allocations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'payment_allocations' AND column_name = 'payment_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_payments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('payment_allocations', 'payment_id', 'platform_payments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'payment_allocations', 'ue_create_parent_owned_rls_policy_v2', 'payment_allocations.payment_id, platform_payments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('platform_invoice_line_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_invoice_line_items' AND column_name = 'invoice_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'platform_invoices' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'platform_invoice_line_items', 'ue_create_parent_owned_rls_policy_v2', 'platform_invoice_line_items.invoice_id, platform_invoices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reconciliation_matches') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reconciliation_matches' AND column_name = 'run_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reconciliation_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('reconciliation_matches', 'run_id', 'reconciliation_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reconciliation_matches', 'ue_create_parent_owned_rls_policy_v2', 'reconciliation_matches.run_id, reconciliation_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('remittance_approvals') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('strike_fund_disbursements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'strike_fund_disbursements' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('strike_fund_disbursements', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'strike_fund_disbursements', 'ue_create_direct_org_rls_policy', 'strike_fund_disbursements.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('board_packets') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'board_packets' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('board_packets', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'board_packets', 'ue_create_direct_org_rls_policy', 'board_packets.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committee_action_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_action_items' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('committee_action_items', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committee_action_items', 'ue_create_direct_org_rls_policy', 'committee_action_items.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committee_documents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('committee_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committee_documents', 'ue_create_direct_org_rls_policy', 'committee_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committee_intelligence_snapshots') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_intelligence_snapshots' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('committee_intelligence_snapshots', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committee_intelligence_snapshots', 'ue_create_direct_org_rls_policy', 'committee_intelligence_snapshots.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committee_meetings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_meetings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('committee_meetings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committee_meetings', 'ue_create_direct_org_rls_policy', 'committee_meetings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committees') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committees' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('committees', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committees', 'ue_create_direct_org_rls_policy', 'committees.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('congress_memberships') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'congress_memberships' AND column_name = 'organization_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'congress_memberships' AND column_name = 'congress_id') THEN
      PERFORM ue_create_multi_party_rls_policy('congress_memberships', 'organization_id', 'congress_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'congress_memberships', 'ue_create_multi_party_rls_policy', 'congress_memberships.organization_id, congress_memberships.congress_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('governance_policies') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'governance_policies' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('governance_policies', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'governance_policies', 'ue_create_direct_org_rls_policy', 'governance_policies.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('icra_governance_flags') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('joint_hs_committees') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'joint_hs_committees' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('joint_hs_committees', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'joint_hs_committees', 'ue_create_direct_org_rls_policy', 'joint_hs_committees.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('board_packet_distributions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'board_packet_distributions' AND column_name = 'packet_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'board_packets' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('board_packet_distributions', 'packet_id', 'board_packets', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'board_packet_distributions', 'ue_create_parent_owned_rls_policy_v2', 'board_packet_distributions.packet_id, board_packets.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('committee_meeting_attendees') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_meeting_attendees' AND column_name = 'meeting_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'committee_meetings' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'committee_meeting_attendees', 'ue_create_parent_owned_rls_policy_v2', 'committee_meeting_attendees.meeting_id, committee_meetings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('council_elections') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('golden_shares') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('governance_events') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('mission_audits') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('reserved_matter_votes') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workbook_governance_lineage_entries') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbook_governance_lineage_entries' AND column_name = 'workbook_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbooks' AND column_name = 'claimed_by_user_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'workbook_governance_lineage_entries', 'ue_create_parent_owned_via_user_rls_policy_v2', 'workbook_governance_lineage_entries.workbook_id, workbooks.claimed_by_user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('deadline_audit_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'deadline_audit_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('deadline_audit_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'deadline_audit_events', 'ue_create_direct_org_rls_policy', 'deadline_audit_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ai_safety_filters') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ai_safety_filters' AND column_name = 'session_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chat_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('ai_safety_filters', 'session_id', 'chat_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ai_safety_filters', 'ue_create_parent_owned_rls_policy_v2', 'ai_safety_filters.session_id, chat_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('correspondence_audit_trail') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'correspondence_audit_trail' AND column_name = 'correspondence_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'correspondence' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'correspondence_audit_trail', 'ue_create_parent_owned_rls_policy_v2', 'correspondence_audit_trail.correspondence_id, correspondence.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('location_tracking_audit') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'location_tracking_audit' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('location_tracking_audit', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'location_tracking_audit', 'ue_create_user_rls_policy', 'location_tracking_audit.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('signature_audit_trail') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'signature_audit_trail' AND column_name = 'document_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'signature_documents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('signature_audit_trail', 'document_id', 'signature_documents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'signature_audit_trail', 'ue_create_parent_owned_rls_policy_v2', 'signature_audit_trail.document_id, signature_documents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('api_integrations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'api_integrations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('api_integrations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'api_integrations', 'ue_create_direct_org_rls_policy', 'api_integrations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('clc_sync_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'clc_sync_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('clc_sync_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'clc_sync_log', 'ue_create_direct_org_rls_policy', 'clc_sync_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_timesheet_batches') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_timesheet_batches' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_timesheet_batches', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_timesheet_batches', 'ue_create_direct_org_rls_policy', 'employer_timesheet_batches.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_accounts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_accounts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_accounts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_accounts', 'ue_create_direct_org_rls_policy', 'external_accounts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_benefit_coverage') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_benefit_coverage' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_benefit_coverage', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_benefit_coverage', 'ue_create_direct_org_rls_policy', 'external_benefit_coverage.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_benefit_dependents') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_benefit_dependents' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_benefit_dependents', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_benefit_dependents', 'ue_create_direct_org_rls_policy', 'external_benefit_dependents.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_benefit_enrollments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_benefit_enrollments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_benefit_enrollments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_benefit_enrollments', 'ue_create_direct_org_rls_policy', 'external_benefit_enrollments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_benefit_plans') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_benefit_plans' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_benefit_plans', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_benefit_plans', 'ue_create_direct_org_rls_policy', 'external_benefit_plans.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_benefit_utilization') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_benefit_utilization' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_benefit_utilization', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_benefit_utilization', 'ue_create_direct_org_rls_policy', 'external_benefit_utilization.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_calendar_connections') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_calendar_connections' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_calendar_connections', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_calendar_connections', 'ue_create_direct_org_rls_policy', 'external_calendar_connections.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_customers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_customers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_customers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_customers', 'ue_create_direct_org_rls_policy', 'external_customers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_departments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_departments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_departments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_departments', 'ue_create_direct_org_rls_policy', 'external_departments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_employees') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_employees' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_employees', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_employees', 'ue_create_direct_org_rls_policy', 'external_employees.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_insurance_beneficiaries') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_insurance_beneficiaries' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_insurance_beneficiaries', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_insurance_beneficiaries', 'ue_create_direct_org_rls_policy', 'external_insurance_beneficiaries.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_insurance_policies') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_insurance_policies' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_insurance_policies', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_insurance_policies', 'ue_create_direct_org_rls_policy', 'external_insurance_policies.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_invoices') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_invoices' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_invoices', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_invoices', 'ue_create_direct_org_rls_policy', 'external_invoices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_lms_completions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_lms_completions' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_lms_completions', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_lms_completions', 'ue_create_direct_org_rls_policy', 'external_lms_completions.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_lms_courses') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_lms_courses' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_lms_courses', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_lms_courses', 'ue_create_direct_org_rls_policy', 'external_lms_courses.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_lms_enrollments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_lms_enrollments' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_lms_enrollments', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_lms_enrollments', 'ue_create_direct_org_rls_policy', 'external_lms_enrollments.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_lms_learners') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_lms_learners' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_lms_learners', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_lms_learners', 'ue_create_direct_org_rls_policy', 'external_lms_learners.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_lms_progress') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_lms_progress' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_lms_progress', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_lms_progress', 'ue_create_direct_org_rls_policy', 'external_lms_progress.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_payments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_payments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_payments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_payments', 'ue_create_direct_org_rls_policy', 'external_payments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('external_positions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'external_positions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('external_positions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'external_positions', 'ue_create_direct_org_rls_policy', 'external_positions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ingestion_batches') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ingestion_batches' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('ingestion_batches', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ingestion_batches', 'ue_create_direct_org_rls_policy', 'ingestion_batches.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('integration_api_keys') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'integration_api_keys' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('integration_api_keys', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'integration_api_keys', 'ue_create_direct_org_rls_policy', 'integration_api_keys.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('integration_configs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'integration_configs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('integration_configs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'integration_configs', 'ue_create_direct_org_rls_policy', 'integration_configs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('integration_partners') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'integration_partners' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('integration_partners', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'integration_partners', 'ue_create_direct_org_rls_policy', 'integration_partners.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('integration_sync_log') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'integration_sync_log' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('integration_sync_log', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'integration_sync_log', 'ue_create_direct_org_rls_policy', 'integration_sync_log.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('integration_webhooks') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'integration_webhooks' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('integration_webhooks', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'integration_webhooks', 'ue_create_direct_org_rls_policy', 'integration_webhooks.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('ingestion_records') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ingestion_records' AND column_name = 'batch_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'ingestion_batches' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('ingestion_records', 'batch_id', 'ingestion_batches', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'ingestion_records', 'ue_create_parent_owned_rls_policy_v2', 'ingestion_records.batch_id, ingestion_batches.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('webhook_receipts') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('clc_organization_sync_log') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('icra_organizations') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('member_arrears') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_arrears' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_arrears', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_arrears', 'ue_create_direct_org_rls_policy', 'member_arrears.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_breaks') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_breaks' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_breaks', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_breaks', 'ue_create_direct_org_rls_policy', 'member_breaks.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_certifications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_certifications' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_certifications', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_certifications', 'ue_create_direct_org_rls_policy', 'member_certifications.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_dues_issues') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_dues_issues' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_dues_issues', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_dues_issues', 'ue_create_direct_org_rls_policy', 'member_dues_issues.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_dues_ledger') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_dues_ledger' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_dues_ledger', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_dues_ledger', 'ue_create_direct_org_rls_policy', 'member_dues_ledger.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_employment') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_employment' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_employment', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_employment', 'ue_create_direct_org_rls_policy', 'member_employment.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_history_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_history_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_history_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_history_events', 'ue_create_direct_org_rls_policy', 'member_history_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_jurisdiction_preferences') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_jurisdiction_preferences' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_jurisdiction_preferences', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_jurisdiction_preferences', 'ue_create_direct_org_rls_policy', 'member_jurisdiction_preferences.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_segments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_segments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('member_segments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_segments', 'ue_create_direct_org_rls_policy', 'member_segments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('org_entitlements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'org_entitlements' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('org_entitlements', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'org_entitlements', 'ue_create_direct_org_rls_policy', 'org_entitlements.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('org_subscriptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'org_subscriptions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('org_subscriptions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'org_subscriptions', 'ue_create_direct_org_rls_policy', 'org_subscriptions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('organization_billing_config') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('pension_members') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pension_members' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pension_members', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pension_members', 'ue_create_direct_org_rls_policy', 'pension_members.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('duplicate_group_members') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'duplicate_group_members' AND column_name = 'group_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'duplicate_groups' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('duplicate_group_members', 'group_id', 'duplicate_groups', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'duplicate_group_members', 'ue_create_parent_owned_rls_policy_v2', 'duplicate_group_members.group_id, duplicate_groups.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('member_location_consent') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'member_location_consent' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('member_location_consent', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'member_location_consent', 'ue_create_user_rls_policy', 'member_location_consent.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_applications') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_applications' AND column_name = 'verified_organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_applications', 'verified_organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_applications', 'ue_create_direct_org_rls_policy', 'pilot_applications.verified_organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('org_configurations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'org_configurations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('org_configurations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'org_configurations', 'ue_create_direct_org_rls_policy', 'org_configurations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('organization_relationships') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('alert_rules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'alert_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('alert_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'alert_rules', 'ue_create_direct_org_rls_policy', 'alert_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('allocation_rules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('allocation_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'allocation_rules', 'ue_create_direct_org_rls_policy', 'allocation_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('anti_scab_violations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'anti_scab_violations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('anti_scab_violations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'anti_scab_violations', 'ue_create_direct_org_rls_policy', 'anti_scab_violations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  IF to_regclass('break_policies') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'break_policies' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('break_policies', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'break_policies', 'ue_create_direct_org_rls_policy', 'break_policies.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('calendar_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'calendar_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('calendar_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'calendar_events', 'ue_create_direct_org_rls_policy', 'calendar_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('calendars') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'calendars' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('calendars', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'calendars', 'ue_create_direct_org_rls_policy', 'calendars.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('chargeback_statements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'chargeback_statements' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('chargeback_statements', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'chargeback_statements', 'ue_create_direct_org_rls_policy', 'chargeback_statements.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('clause_comparisons') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'clause_comparisons' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('clause_comparisons', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'clause_comparisons', 'ue_create_direct_org_rls_policy', 'clause_comparisons.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cms_pages') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cms_pages' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cms_pages', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cms_pages', 'ue_create_direct_org_rls_policy', 'cms_pages.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('cnesst_filings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cnesst_filings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('cnesst_filings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'cnesst_filings', 'ue_create_direct_org_rls_policy', 'cnesst_filings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('compliance_alerts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'compliance_alerts' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('compliance_alerts', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'compliance_alerts', 'ue_create_direct_org_rls_policy', 'compliance_alerts.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('contract_covered_orgs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'contract_covered_orgs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('contract_covered_orgs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'contract_covered_orgs', 'ue_create_direct_org_rls_policy', 'contract_covered_orgs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('correspondence') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'correspondence' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('correspondence', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'correspondence', 'ue_create_direct_org_rls_policy', 'correspondence.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('course_registrations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'course_registrations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('course_registrations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'course_registrations', 'ue_create_direct_org_rls_policy', 'course_registrations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('course_sessions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'course_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('course_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'course_sessions', 'ue_create_direct_org_rls_policy', 'course_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('customer_onboarding_milestones') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('data_quality_warnings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'data_quality_warnings' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('data_quality_warnings', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'data_quality_warnings', 'ue_create_direct_org_rls_policy', 'data_quality_warnings.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('defensibility_packs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'defensibility_packs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('defensibility_packs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'defensibility_packs', 'ue_create_direct_org_rls_policy', 'defensibility_packs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dispatch_requests') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dispatch_requests' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('dispatch_requests', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dispatch_requests', 'ue_create_direct_org_rls_policy', 'dispatch_requests.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dispatch_rules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dispatch_rules' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('dispatch_rules', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dispatch_rules', 'ue_create_direct_org_rls_policy', 'dispatch_rules.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('duplicate_groups') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'duplicate_groups' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('duplicate_groups', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'duplicate_groups', 'ue_create_direct_org_rls_policy', 'duplicate_groups.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_contacts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_contacts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_contacts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_contacts', 'ue_create_direct_org_rls_policy', 'employer_contacts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_execution_artifacts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_execution_artifacts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_execution_artifacts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_execution_artifacts', 'ue_create_direct_org_rls_policy', 'employer_execution_artifacts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_execution_compliance_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_execution_compliance_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_execution_compliance_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_execution_compliance_events', 'ue_create_direct_org_rls_policy', 'employer_execution_compliance_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_execution_replays') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_execution_replays' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_execution_replays', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_execution_replays', 'ue_create_direct_org_rls_policy', 'employer_execution_replays.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_timesheet_entries') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_timesheet_entries' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employer_timesheet_entries', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_timesheet_entries', 'ue_create_direct_org_rls_policy', 'employer_timesheet_entries.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('employers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employers', 'ue_create_direct_org_rls_policy', 'employers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('exit_interview_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'exit_interview_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('exit_interview_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'exit_interview_events', 'ue_create_direct_org_rls_policy', 'exit_interview_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('exit_interviews') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'exit_interviews' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('exit_interviews', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'exit_interviews', 'ue_create_direct_org_rls_policy', 'exit_interviews.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('federations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'federations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('federations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'federations', 'ue_create_direct_org_rls_policy', 'federations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('fee_adjustments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'fee_adjustments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('fee_adjustments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'fee_adjustments', 'ue_create_direct_org_rls_policy', 'fee_adjustments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('gdpr_data_requests') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'gdpr_data_requests' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('gdpr_data_requests', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'gdpr_data_requests', 'ue_create_user_rls_policy', 'gdpr_data_requests.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('holidays') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'holidays' AND column_name = 'organization_id') THEN
      PERFORM ue_create_mixed_global_tenant_rls_policy('holidays', 'organization_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'holidays', 'ue_create_mixed_global_tenant_rls_policy', 'holidays.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('icra_assessments') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('icra_assessment_answers') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('knowledge_base') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'knowledge_base' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('knowledge_base', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'knowledge_base', 'ue_create_direct_org_rls_policy', 'knowledge_base.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('kpi_configurations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'kpi_configurations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('kpi_configurations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'kpi_configurations', 'ue_create_direct_org_rls_policy', 'kpi_configurations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('meeting_rooms') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'meeting_rooms' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('meeting_rooms', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'meeting_rooms', 'ue_create_direct_org_rls_policy', 'meeting_rooms.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('mobile_devices') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'mobile_devices' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('mobile_devices', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'mobile_devices', 'ue_create_direct_org_rls_policy', 'mobile_devices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('negotiations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'negotiations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('negotiations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'negotiations', 'ue_create_direct_org_rls_policy', 'negotiations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('organizer_tasks') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'organizer_tasks' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('organizer_tasks', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'organizer_tasks', 'ue_create_direct_org_rls_policy', 'organizer_tasks.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pay_equity_exercises') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pay_equity_exercises' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pay_equity_exercises', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pay_equity_exercises', 'ue_create_direct_org_rls_policy', 'pay_equity_exercises.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_checklist_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_checklist_items' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_checklist_items', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_checklist_items', 'ue_create_direct_org_rls_policy', 'pilot_checklist_items.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_demo_seeds') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_demo_seeds' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_demo_seeds', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_demo_seeds', 'ue_create_direct_org_rls_policy', 'pilot_demo_seeds.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_enrollments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_enrollments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_mixed_global_tenant_rls_policy('pilot_enrollments', 'organization_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_enrollments', 'ue_create_mixed_global_tenant_rls_policy', 'pilot_enrollments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_events', 'ue_create_direct_org_rls_policy', 'pilot_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_feedback') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_feedback' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_feedback', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_feedback', 'ue_create_direct_org_rls_policy', 'pilot_feedback.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('pilot_milestones') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'pilot_milestones' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('pilot_milestones', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'pilot_milestones', 'ue_create_direct_org_rls_policy', 'pilot_milestones.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('policy_rules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'policy_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('policy_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'policy_rules', 'ue_create_direct_org_rls_policy', 'policy_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('poll_votes') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'poll_votes' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('poll_votes', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'poll_votes', 'ue_create_direct_org_rls_policy', 'poll_votes.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('polls') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'polls' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('polls', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'polls', 'ue_create_direct_org_rls_policy', 'polls.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('preventive_withdrawals') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'preventive_withdrawals' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('preventive_withdrawals', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'preventive_withdrawals', 'ue_create_direct_org_rls_policy', 'preventive_withdrawals.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('push_devices') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'push_devices' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('push_devices', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'push_devices', 'ue_create_direct_org_rls_policy', 'push_devices.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('recognition_award_types') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'recognition_award_types' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('recognition_award_types', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'recognition_award_types', 'ue_create_direct_org_rls_policy', 'recognition_award_types.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('recognition_awards') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'recognition_awards' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('recognition_awards', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'recognition_awards', 'ue_create_direct_org_rls_policy', 'recognition_awards.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('recognition_programs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'recognition_programs' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('recognition_programs', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'recognition_programs', 'ue_create_direct_org_rls_policy', 'recognition_programs.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reports') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reports' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reports', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reports', 'ue_create_direct_org_rls_policy', 'reports.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('reward_redemptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'reward_redemptions' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('reward_redemptions', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'reward_redemptions', 'ue_create_direct_org_rls_policy', 'reward_redemptions.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('right_of_refusal_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'right_of_refusal_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('right_of_refusal_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'right_of_refusal_events', 'ue_create_direct_org_rls_policy', 'right_of_refusal_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('satisfaction_surveys') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'satisfaction_surveys' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('satisfaction_surveys', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'satisfaction_surveys', 'ue_create_direct_org_rls_policy', 'satisfaction_surveys.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('security_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'security_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('security_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'security_events', 'ue_create_direct_org_rls_policy', 'security_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('security_posture_checks') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'security_posture_checks' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('security_posture_checks', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'security_posture_checks', 'ue_create_direct_org_rls_policy', 'security_posture_checks.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('social_posts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'social_posts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('social_posts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'social_posts', 'ue_create_direct_org_rls_policy', 'social_posts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('sso_providers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'sso_providers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('sso_providers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'sso_providers', 'ue_create_direct_org_rls_policy', 'sso_providers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('steward_assignments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'steward_assignments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('steward_assignments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'steward_assignments', 'ue_create_direct_org_rls_policy', 'steward_assignments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('stewards') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'stewards' AND column_name = 'org_id') THEN
      PERFORM ue_create_direct_org_rls_policy('stewards', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'stewards', 'ue_create_direct_org_rls_policy', 'stewards.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('strategic_goals') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'strategic_goals' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('strategic_goals', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'strategic_goals', 'ue_create_direct_org_rls_policy', 'strategic_goals.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  IF to_regclass('survey_answers') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'survey_answers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('survey_answers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'survey_answers', 'ue_create_direct_org_rls_policy', 'survey_answers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('survey_questions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'survey_questions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('survey_questions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'survey_questions', 'ue_create_direct_org_rls_policy', 'survey_questions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('survey_responses') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'survey_responses' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('survey_responses', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'survey_responses', 'ue_create_direct_org_rls_policy', 'survey_responses.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('surveys') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'surveys' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('surveys', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'surveys', 'ue_create_direct_org_rls_policy', 'surveys.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('training_courses') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'training_courses' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('training_courses', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'training_courses', 'ue_create_direct_org_rls_policy', 'training_courses.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('training_programs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'training_programs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('training_programs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'training_programs', 'ue_create_direct_org_rls_policy', 'training_programs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('transaction_fee_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'transaction_fee_events' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('transaction_fee_events', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'transaction_fee_events', 'ue_create_direct_org_rls_policy', 'transaction_fee_events.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('transaction_fee_rules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'transaction_fee_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('transaction_fee_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'transaction_fee_rules', 'ue_create_direct_org_rls_policy', 'transaction_fee_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('trend_analyses') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'trend_analyses' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('trend_analyses', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'trend_analyses', 'ue_create_direct_org_rls_policy', 'trend_analyses.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('user_signatures') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'user_signatures' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('user_signatures', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'user_signatures', 'ue_create_user_rls_policy', 'user_signatures.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('voting_sessions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voting_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('voting_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'voting_sessions', 'ue_create_direct_org_rls_policy', 'voting_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('wcb_employer_assessments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'wcb_employer_assessments' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('wcb_employer_assessments', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'wcb_employer_assessments', 'ue_create_direct_org_rls_policy', 'wcb_employer_assessments.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('worksites') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'worksites' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('worksites', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'worksites', 'ue_create_direct_org_rls_policy', 'worksites.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('alert_executions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'alert_executions' AND column_name = 'alert_rule_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'alert_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('alert_executions', 'alert_rule_id', 'alert_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'alert_executions', 'ue_create_parent_owned_rls_policy_v2', 'alert_executions.alert_rule_id, alert_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('allocation_runs') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('allocation_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'allocation_runs', 'ue_create_direct_org_rls_policy', 'allocation_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('allocation_basis_snapshots') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_basis_snapshots' AND column_name = 'run_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('allocation_basis_snapshots', 'run_id', 'allocation_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'allocation_basis_snapshots', 'ue_create_parent_owned_rls_policy_v2', 'allocation_basis_snapshots.run_id, allocation_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('allocation_rule_versions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_rule_versions' AND column_name = 'rule_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('allocation_rule_versions', 'rule_id', 'allocation_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'allocation_rule_versions', 'ue_create_parent_owned_rls_policy_v2', 'allocation_rule_versions.rule_id, allocation_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('allocation_run_lines') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_run_lines' AND column_name = 'run_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'allocation_runs' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('allocation_run_lines', 'run_id', 'allocation_runs', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'allocation_run_lines', 'ue_create_parent_owned_rls_policy_v2', 'allocation_run_lines.run_id, allocation_runs.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('clause_embeddings') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'clause_embeddings' AND column_name = 'clause_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'cba_clauses' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('clause_embeddings', 'clause_id', 'cba_clauses', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'clause_embeddings', 'ue_create_parent_owned_rls_policy_v2', 'clause_embeddings.clause_id, cba_clauses.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('clause_library_tags') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'clause_library_tags' AND column_name = 'clause_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'source_organization_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'shared_with_org_ids') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'sharing_level') THEN
      PERFORM ue_create_shared_library_child_rls_policy('clause_library_tags', 'clause_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'clause_library_tags', 'ue_create_shared_library_child_rls_policy', 'clause_library_tags.clause_id, shared_clause_library.source_organization_id, shared_clause_library.shared_with_org_ids, shared_clause_library.sharing_level';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('commercial_contracts') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'commercial_contracts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_direct_org_rls_policy('commercial_contracts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'commercial_contracts', 'ue_create_direct_org_rls_policy', 'commercial_contracts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('contract_line_items') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'contract_line_items' AND column_name = 'contract_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'commercial_contracts' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('contract_line_items', 'contract_id', 'commercial_contracts', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'contract_line_items', 'ue_create_parent_owned_rls_policy_v2', 'contract_line_items.contract_id, commercial_contracts.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('correspondence_recipients') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'correspondence_recipients' AND column_name = 'correspondence_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'correspondence' AND column_name = 'organization_id') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'correspondence_recipients', 'ue_create_parent_owned_rls_policy_v2', 'correspondence_recipients.correspondence_id, correspondence.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('data_subject_access_requests') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'data_subject_access_requests' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('data_subject_access_requests', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'data_subject_access_requests', 'ue_create_user_rls_policy', 'data_subject_access_requests.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('dispatch_assignments') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dispatch_assignments' AND column_name = 'request_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'dispatch_requests' AND column_name = 'org_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('dispatch_assignments', 'request_id', 'dispatch_requests', 'org_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'dispatch_assignments', 'ue_create_parent_owned_rls_policy_v2', 'dispatch_assignments.request_id, dispatch_requests.org_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('employer_reports') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employer_reports' AND column_name = 'employer_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'employers' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('employer_reports', 'employer_id', 'employers', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'employer_reports', 'ue_create_parent_owned_rls_policy_v2', 'employer_reports.employer_id, employers.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('geofence_events') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'geofence_events' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('geofence_events', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'geofence_events', 'ue_create_user_rls_policy', 'geofence_events.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('geofences') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'geofences' AND column_name = 'union_local_id') THEN
      PERFORM ue_create_direct_org_rls_policy('geofences', 'union_local_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'geofences', 'ue_create_direct_org_rls_policy', 'geofences.union_local_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('location_tracking') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'location_tracking' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('location_tracking', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'location_tracking', 'ue_create_user_rls_policy', 'location_tracking.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('policy_evaluations') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'policy_evaluations' AND column_name = 'rule_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'policy_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('policy_evaluations', 'rule_id', 'policy_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'policy_evaluations', 'ue_create_parent_owned_rls_policy_v2', 'policy_evaluations.rule_id, policy_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('policy_exceptions') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'policy_exceptions' AND column_name = 'rule_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'policy_rules' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('policy_exceptions', 'rule_id', 'policy_rules', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'policy_exceptions', 'ue_create_parent_owned_rls_policy_v2', 'policy_exceptions.rule_id, policy_rules.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('provincial_data_handling') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'provincial_data_handling' AND column_name = 'user_id') THEN
      PERFORM ue_create_user_rls_policy('provincial_data_handling', 'user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'provincial_data_handling', 'ue_create_user_rls_policy', 'provincial_data_handling.user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('shared_clause_library') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'source_organization_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'sharing_level') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'shared_clause_library' AND column_name = 'shared_with_org_ids') THEN
    ...
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'shared_clause_library', 'ue_create_shared_library_rls_policy', 'shared_clause_library.source_organization_id, shared_clause_library.sharing_level, shared_clause_library.shared_with_org_ids';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('tentative_agreements') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'tentative_agreements' AND column_name = 'negotiation_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'negotiations' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('tentative_agreements', 'negotiation_id', 'negotiations', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'tentative_agreements', 'ue_create_parent_owned_rls_policy_v2', 'tentative_agreements.negotiation_id, negotiations.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('union_density') IS NOT NULL THEN
...
END $$;
DO $$ BEGIN
  IF to_regclass('voter_eligibility') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voter_eligibility' AND column_name = 'session_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voting_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('voter_eligibility', 'session_id', 'voting_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'voter_eligibility', 'ue_create_parent_owned_rls_policy_v2', 'voter_eligibility.session_id, voting_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('votes') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'votes' AND column_name = 'session_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voting_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('votes', 'session_id', 'voting_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'votes', 'ue_create_parent_owned_rls_policy_v2', 'votes.session_id, voting_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('voting_options') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voting_options' AND column_name = 'session_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'voting_sessions' AND column_name = 'organization_id') THEN
      PERFORM ue_create_parent_owned_rls_policy_v2('voting_options', 'session_id', 'voting_sessions', 'organization_id', FALSE);
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'voting_options', 'ue_create_parent_owned_rls_policy_v2', 'voting_options.session_id, voting_sessions.organization_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('workbook_memory_holders') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbook_memory_holders' AND column_name = 'workbook_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbooks' AND column_name = 'claimed_by_user_id') THEN
      PERFORM ue_create_parent_owned_via_user_rls_policy_v2('workbook_memory_holders', 'workbook_id', 'workbooks', 'claimed_by_user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'workbook_memory_holders', 'ue_create_parent_owned_via_user_rls_policy_v2', 'workbook_memory_holders.workbook_id, workbooks.claimed_by_user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('workbook_modules') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbook_modules' AND column_name = 'workbook_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbooks' AND column_name = 'claimed_by_user_id') THEN
      PERFORM ue_create_parent_owned_via_user_rls_policy_v2('workbook_modules', 'workbook_id', 'workbooks', 'claimed_by_user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'workbook_modules', 'ue_create_parent_owned_via_user_rls_policy_v2', 'workbook_modules.workbook_id, workbooks.claimed_by_user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('workbook_purchases') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbook_purchases' AND column_name = 'workbook_id') AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbooks' AND column_name = 'claimed_by_user_id') THEN
      PERFORM ue_create_parent_owned_via_user_rls_policy_v2('workbook_purchases', 'workbook_id', 'workbooks', 'claimed_by_user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'workbook_purchases', 'ue_create_parent_owned_via_user_rls_policy_v2', 'workbook_purchases.workbook_id, workbooks.claimed_by_user_id';
    END IF;
  END IF;
END $$;
DO $$ BEGIN
  IF to_regclass('workbooks') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = ANY(current_schemas(false)) AND table_name = 'workbooks' AND column_name = 'claimed_by_user_id') THEN
      PERFORM ue_create_user_rls_policy('workbooks', 'claimed_by_user_id');
    ELSE
      RAISE EXCEPTION 'RLS enforcement geometry incomplete for table %: helper % requires column(s) [%] which are not all present in this environment', 'workbooks', 'ue_create_user_rls_policy', 'workbooks.claimed_by_user_id';
    END IF;
  END IF;
END $$;
...
DO $$ BEGIN
  IF to_regclass('ai_rate_limits') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('analytics_scheduled_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('icra_anonymized_metrics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('impact_metrics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mobile_analytics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('page_analytics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('usage_aggregates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('usage_meters') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('user_engagement_scores') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('claims') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_deadlines') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_documents') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workplace_incidents') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_inspections') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('hazard_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_committee_meetings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_training_records') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ppe_equipment') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_audits') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('injury_logs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('corrective_actions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('safety_certifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('message_threads') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('message_participants') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('message_read_receipts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('message_notifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cross_org_access_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_assignments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('deadline_reassignment_convergence') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('deadline_alerts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('deadline_extensions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('deadline_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('case_studies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('claim_precedent_analysis') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fee_settlement_lines') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_approvals') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_communications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_stages') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_workflows') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signature_workflows') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('union_representation_votes') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workflow_executions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('arbitration_decisions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('bargaining_team_members') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_contacts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_footnotes') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_agreements') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_benchmark_snapshots') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_clauses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_documents') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_extraction_runs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_findings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_freshness_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_ingestion_jobs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_review_decisions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_sources') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_intel_wage_adjustments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cba_version_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_bargaining_trends') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('deadline_reminder_executions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fee_settlement_batches') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('grievance_responses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('chatbot_analytics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('chatbot_suggestions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('communication_analytics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('communication_channels') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('communication_preferences_phase4') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mobile_notifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('notification_bounces') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('notification_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('notification_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organizing_campaign_milestones') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sms_opt_outs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sms_rate_limits') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('band_council_consent') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_campaigns') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_communications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('newsletter_engagement') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sms_campaign_recipients') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_notifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('award_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cms_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('communication_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('document_search_index') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_document_files') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_document_libraries') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_document_permissions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_document_sites') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('newsletter_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('push_notification_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('report_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signature_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sms_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('arbitrator_profiles') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pending_profiles') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pricing_template_modules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pricing_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('swiss_cold_storage') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('transfer_pricing_documentation') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_budgets') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('billing_adjustments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('billing_invoices') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('billing_payments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('donation_receipts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('donations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dues_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('employer_payroll_adjustments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('union_dues_receipts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('union_dues_year_end') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('account_balance_reconciliation') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('billing_terms') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_chart_of_accounts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_remittances') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('payment_classification_policy') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('payment_routing_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('rl1_tax_slips') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('separated_payment_transactions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('stripe_connect_accounts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('subscription_plans') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('t4a_tax_slips') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('tax_year_end_processing') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('accounts_payable') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_chunks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_feedback') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_feedback_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_queries') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_query_logs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ai_usage_by_tenant') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('arrears') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('arrears_cases') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('attestation_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('blockchain_audit_anchors') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('budget_line_items') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('case_summaries') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('certification_applications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('compliance_validations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cope_contributions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cra_xml_batches') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('customer_acquisition') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('digital_signatures') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dues_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('elected_officials') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('encryption_keys') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('equity_snapshots') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('expense_approvals') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('expense_requests') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fund_eligibility') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('hardship_applications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('hw_benefit_claims') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('hw_benefit_enrollments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('hw_benefit_plans') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('jurisdiction_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('jurisdiction_rules_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('jurisdiction_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('legislation_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_demographics') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_dues_assignments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_political_participation') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('members_with_pii') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mrr_snapshots') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_hierarchy_audit') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_tree') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organizing_activities') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organizing_volunteers') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pay_equity_complaints') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pension_actuarial_valuations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pension_hours_banks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pension_trustee_boards') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('picket_attendance') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('picket_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pii_access_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('political_activities') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('political_campaigns') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('public_donations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('revenue_cohorts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('statcan_submissions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('statutory_holidays') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('stipend_disbursements') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('strike_funds') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('subscription_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('tax_slips') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('tax_year_configurations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('tenant_management_view') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('transaction_clc_mappings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('trust_compliance_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('trusted_certificate_authorities') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_annual_remittance_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_certification_expiry_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_cope_member_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_course_session_dashboard') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_critical_deadlines') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_elected_official_engagement') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_equity_statistics_anonymized') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_hw_claims_aging') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_legislative_priorities') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_member_benefit_eligibility') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_member_training_transcript') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_organizing_campaign_dashboard') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_pay_equity_pipeline') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_pending_remittances') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_pension_funding_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_political_campaign_dashboard') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_tax_slip_summary') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_training_program_progress') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('v_workplace_contact_map') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('vendor_invoices') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('vote_merkle_tree') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_auditors') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_key_access_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_session_auditors') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_session_keys') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('board_packet_templates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('governance_bylaws') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('governance_signatories') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ue_governance_job_cancellation_audit_event') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ue_governance_job_cancellation_request') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ue_governance_job_execution_state') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ue_governance_job_reconciliation_pass') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('board_packet_sections') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('committee_memberships') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('conflict_review_committee') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('accessibility_audits') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('financial_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('certification_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('conflict_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('currency_enforcement_audit') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('firewall_compliance_audit') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fmv_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fx_rate_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signature_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('strike_fund_payment_audit') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('voting_audit_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('whiplash_prevention_audit') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_webhook_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('erp_connectors') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_calendar_attendees') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_calendar_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_calendar_recurring_patterns') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_calendars') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_beneficiaries') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_contributions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_estimates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_plans') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_service_credits') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('integration_sync_schedules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('job_classifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('job_postings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('job_saved') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mobile_sync_queue') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sync_jobs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('webhook_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_data_sync_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('foreign_workers') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('integration_sync_logs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lrb_sync_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signature_webhooks_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('external_pension_members') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_addresses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_consents') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_contact_preferences') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_employment_details') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_leaves') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('member_relationship_scores') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_benchmark_snapshots') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_contacts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_sharing_settings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('role_tenure_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_memberships') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('gss_applications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('indigenous_member_data') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organization_sharing_grants') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('applications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('org_usage') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('accessibility_issues') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('accessibility_test_suites') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('accessibility_user_testing') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('alert_escalations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('api_access_tokens') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('calendar_sharing') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('card_signing_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clause_comparisons_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_api_config') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_per_capita_benchmarks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cms_blocks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cms_navigation_menus') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('comparative_analyses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('currency_exchange_rates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_anonymization_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_processing_records') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_residency_configs') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_retention_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dsr_requests') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dunning_cases') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('employer_responses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('employment_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('event_attendees') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('event_check_ins') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('event_registrations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('event_reminders') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('exit_interview_sessions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('field_notes') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('field_organizer_activities') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('icra_operational_indicators') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('icra_benchmark_groups') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('international_addresses') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('legal_holds') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mfa_configurations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mobile_app_config') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('nlrb_clrb_filings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organizer_impacts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('organizing_contacts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('outreach_enrollments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('outreach_sequences') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('outreach_steps_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pack_download_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pci_dss_cardholder_data_flow') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pci_dss_encryption_keys') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pci_dss_quarterly_scans') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pci_dss_requirements') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pci_dss_saq_assessments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('program_enrollments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('public_content') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('public_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('report_delivery_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('report_executions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('report_shares') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('retention_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('scheduled_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('scim_configurations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('scim_events_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('segment_exports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('shopify_config') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('social_engagement') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('social_feeds') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sso_sessions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ue_policy_bindings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('website_settings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ab_test_assignments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ab_test_events') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('access_justification_requests') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('address_change_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('address_validation_cache') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('alert_actions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('alert_conditions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('alert_recipients') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('arms_length_verification') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('automation_execution_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('automation_schedules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('autopay_settings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('award_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('band_councils') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('bank_of_canada_rates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('benchmark_categories') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('benchmark_data') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('benefit_comparisons') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('blind_trust_registry') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('break_glass_activations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('break_glass_system') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('certification_alerts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('certification_compliance_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('certification_types') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_oauth_tokens') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('clc_union_density') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('conflict_disclosures') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('conflict_of_interest_policy') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('conflict_training') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('continuing_education') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('contract_amendments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('contract_rate_cards') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('contribution_rates') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cost_of_living_data') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('country_address_formats') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cpi_adjusted_pricing') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cpi_data') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('cross_border_transactions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('currency_enforcement_policy') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('currency_enforcement_violations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_classification_policy') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('data_classification_registry') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('disaster_recovery_drills') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dsr_activity_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dunning_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('dunning_steps') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('emergency_declarations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('employer_access_attempts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('feature_flags') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_executives') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_meetings') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('federation_resources') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('firewall_access_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('firewall_violations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fmv_benchmarks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fmv_policy') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('fmv_violations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('independent_appraisals') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('indigenous_data_access_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('indigenous_data_sharing_agreements') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('journal_entry_lines') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('key_holder_registry') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('knowledge_base_articles') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('license_renewals') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lmbp_compliance_alerts') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lmbp_compliance_reports') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lmbp_letters') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('location_deletion_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('location_tracking_config') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lrb_agreements') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lrb_employers') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('lrb_unions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('mentorships') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('movement_trends') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('negotiation_sessions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pack_verification_log') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('precedent_citations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('precedent_tags') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pricing_discount_rules') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('pricing_regional_deployments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('privacy_breaches') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('procurement_bids') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('procurement_requests') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('provincial_privacy_config') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('push_deliveries') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('recovery_time_objectives') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('recusal_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('segment_executions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signature_verification') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('signers') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('sla_policies') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('staff_certifications') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('t106_filing_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('testimonials') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ticket_comments') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('ticket_history') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('traditional_knowledge_registry') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('transaction_currency_conversions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('union_only_data_tags') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('user_uuid_mapping') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('wage_benchmarks') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('wage_progressions') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('wcag_success_criteria') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('weekly_threshold_tracking') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('whiplash_violations') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workbook_continuity_breakpoints') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workbook_modernization_alignment') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workbook_stewardship_signals') IS NOT NULL THEN
...
DO $$ BEGIN
  IF to_regclass('workbook_transformation_roadmap') IS NOT NULL THEN
