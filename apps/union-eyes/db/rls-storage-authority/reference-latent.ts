...
export const referenceLatentEntries: StorageAuthorityEntry[] = [
  ...
  },
  {
    table: "policy_rules",
    classification: "NEEDS_REVIEW",
    reason: "ROUND 40 EXCEPTION (2026-09-04, SIMPLE_TENANT:org:NORMAL cohort): not closed despite the normal-lane census hint. policyRules is live through app/api/governance/policies/evaluate/route.ts -> lib/services/policy-engine.ts evaluate(), but evaluate() reads active policyRules by ruleType/category/status/enforced without an organizationId predicate or an independently proven global-reference-data contract. The generated PolicyRulesViewSet in backend/compliance/views.py is now unconditionally denied via DenyAllPermission, but the TypeScript authority boundary remains unresolved. Keep NEEDS_REVIEW until this is either proven global/system policy data or made tenant-scoped.",
