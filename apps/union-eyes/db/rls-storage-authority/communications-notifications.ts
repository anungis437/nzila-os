...
export const communicationsNotificationsEntries: StorageAuthorityEntry[] = [
  ...
  },
  {
    ...
    classification: "NEEDS_REVIEW",
    reason: "ROUND 42 EXCEPTION (PARENT_OWNED:parent:HIGH, ejected): two parent FKs \u2014 list_id -> newsletterDistributionLists (NEEDS_REVIEW, though schema-clear with a direct organizationId) and profile_id -> profiles.userId (NEEDS_REVIEW, global identity table). DEFECT FOUND, NOT FIXED (design ambiguity, out of bounded scope): app/api/communications/distribution-lists/[id]/subscribers/route.ts uses lib/api/crud-factory.ts with { itemRoute: true, orgScoped: true } \u2014 but (1) this table has no organizationId column of its own, so orgScoped:true silently no-ops (same class of bug fixed directly in tentative_agreements this round); (2) the route's folder path puts the LIST's id in the single [id] URL segment (distribution-lists/[id]/subscribers), yet crud-factory's itemRoute mode expects params.id to identify a SUBSCRIBER row (eq(newsletterListSubscribers.id, id)) \u2014 meaning every real distribution-list id passed here would essentially never match a subscriber row. Unclear whether the intended design is 'list all subscribers for a list' (collection semantics) or 'read one subscriber' (item semantics); fixing this requires a product-level decision, not a mechanical org-predicate fix. Recommended next cohort: resolve the intended route shape with product input, then apply the same parent-verification fix pattern used for tentative_agreements.",
