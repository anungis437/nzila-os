...
export const organizationsMembershipEntries: StorageAuthorityEntry[] = [
  ...
  },
  {
    ...
    classification: "TENANT_RLS_REQUIRED",
    reason: "Resolved 2026-09-02 (round 17): no organization_id COLUMN exists (a prospective union has no org record yet at application time) — the owning organization instead lives in a `responses.organizationId` JSONB field, per lib/pilot/pilot-ownership.ts's own design doc. Per-item routes ([id], artifacts, commercial-transition, commercialization-timeline, intelligence, package-export, proposal, reference-profile) all wrap their handler in withPilotOwnership()/enforcePilotOwnership(), which reads that JSONB field and denies cross-org access unless the actor holds system_admin+ (matching PILOT_PLATFORM_ACCESS_MIN_LEVEL) — a purpose-built, already-hardened ownership gate (a prior 'UE Hardening Wave' effort, not this PR). REAL BUG FOUND+FIXED this round: the collection route's GET (app/api/pilot/apply/route.ts, crudRoutes) had orgScoped:false (mechanically required — no column to filter on) but readRole:'steward', an ordinary per-org tier — letting ANY steward at ANY organization list every OTHER organization's sensitive pilot-intake data (contact info, member counts, internal challenges/goals) with zero filtering. Fixed: readRole raised to 'system_admin', matching the per-item routes' own established platform-tier threshold. POST (public lead-intake form submission, no auth wrapper) is unauthenticated by design — confirmed correct, not a gap. Regression test: app/api/pilot/__tests__/pilot-apply-collection-authority-guard.test.ts. NOTE: classified TENANT_RLS_REQUIRED as the closest fit despite lacking a real column — a DB-level RLS policy is not directly expressible without a generated/expression column over responses->>'organizationId'; enforcement today is entirely at the app layer via the ownership module above, which is why review priority is NONE, not left open.",
    supportingCapability: ["app/api/pilot/apply/route.ts","lib/pilot/pilot-ownership.ts","app/api/pilot/apply/[id]/route.ts"],
    requiredRuntimePrivileges: ["SELECT","INSERT","UPDATE"],
    requiredSystemPrivileges: [],
