...
export const storageAuthorityManifest: StorageAuthorityEntry[] = [
  ...
  },
  {
    table: 'organizations',
    classification: 'TENANT_RLS_REQUIRED',
    reason: "0108-baseline table (direct org policy keyed on the org's own id per 0108's ue_create_direct_org_rls_policy('organizations', 'id', FALSE) call — a tenant sees only its own org row). Evidence census across 173 real files: confirmed .insert(organizations) (6 sites, org creation/onboarding) and .update(organizations) (8 sites, org settings/profile), but NO .delete(organizations) call site found anywhere — organizations are not hard-deleted in this codebase (soft-delete/deactivation pattern), so DELETE is deliberately excluded rather than defaulted to FULL_DML.",
    supportingCapability: ["lib/organization-utils.ts"],
  ...
  },
  {
    ...
    classification: 'LATENT_UNREACHABLE',
    reason: "0108-baseline table, but zero non-test references to the 'safetyTrainingRecords' Drizzle export exist anywhere in app/, actions/, lib/, or services/ (verified by name-search). Already RLS-protected at the DB level by 0108 regardless of this manifest entry. Revisit if any code starts querying it.",
    supportingCapability: [],
    requiredRuntimePrivileges: [],
    requiredSystemPrivileges: [],
    invocationAuthority: 'NONE',
