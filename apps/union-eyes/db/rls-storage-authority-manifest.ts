...
export const storageAuthorityManifest: StorageAuthorityEntry[] = [
  ...
  },
  {
    ...
    classification: 'TENANT_RLS_REQUIRED',
    reason: "Manually verified 2026-09-01: direct NOT NULL organization_id column (db/schema/domains/claims/workflows.ts, which is the definition @/db/schema's barrel actually re-exports — confirmed live routes app/api/grievances/[id]/documents/route.ts and app/api/grievances/[id]/route.ts, plus lib/document-management-system.ts, all query .organizationId successfully). SEPARATE FINDING (not an RLS gap, but flag for follow-up): two OTHER files also declare a same-named 'grievanceDocuments' pgTable for this exact physical table — db/schema/domains/claims/grievance-lifecycle.ts (no organization_id column at all) and db/schema/grievance-workflow-schema.ts — a 3-way duplicate schema declaration with inconsistent columns is a real schema-drift risk independent of RLS.",
    supportingCapability: ["app/api/grievances/[id]/documents/route.ts", "app/api/grievances/[id]/route.ts", "lib/document-management-system.ts", "db/schema/domains/claims/workflows.ts"],
    requiredRuntimePrivilege: 'FULL_DML',
    reviewPriority: 'HIGH',
  },
  {
