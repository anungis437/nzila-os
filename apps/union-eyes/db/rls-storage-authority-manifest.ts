...
export const storageAuthorityManifest: StorageAuthorityEntry[] = [
  ...
  },
  {
    ...
    reason: "Verified 2026-09-01 (round 6, Phase-3A priority tranche + reachability-scan correction): no pgTable('organizing_contacts', ...) declaration exists anywhere in apps/union-eyes/db/schema outside services/financial-service — this table belongs solely to financial-service's own database boundary (its own drizzle.config.ts/package.json/DATABASE_URL), mirroring the deadline_alerts precedent. Not reachable from the union-eyes tenant runtime at all.",
    supportingCapability: [],
    requiredRuntimePrivilege: 'NONE',
    reviewPriority: 'NONE',
  },
  ...
  },
  {
    ...
    classification: 'SEPARATE_DATABASE_BOUNDARY',
    reason: "Verified 2026-09-01 (round 6, Phase-3A priority tranche + reachability-scan correction): no pgTable('stripe_webhook_events', ...) declaration exists anywhere in apps/union-eyes/db/schema outside services/financial-service — used only by that service's own payment-processing/reconciliation-engine against its own separate database. Not reachable from the union-eyes tenant runtime.",
    supportingCapability: [],
    requiredRuntimePrivilege: 'NONE',
    reviewPriority: 'NONE',
  },
