...
export const storageAuthorityManifest: StorageAuthorityEntry[] = [
  ...
  },
  {
    table: 'campaigns',
    classification: 'TENANT_RLS_REQUIRED',
    reason: "Verified 2026-09-01 against LIVE staging DB: 27 columns, organization_id present, matching db/schema/domains/communications/campaigns.ts. Heavily reachable (40+ real references across the communications dashboard, API routes, and webhooks) - the vast majority of consumers already use the canonical declaration correctly. ONE real consumer, lib/workers/message-queue-processor.ts, still imports the stale db/schema/phase-4-messaging-schema.ts declaration, which has sentCount/failedCount/skippedCount/totalRecipients columns that do NOT physically exist - its campaign-stat-increment UPDATE statements throw on every call. NOT fixed in this pass: the canonical schema replaced discrete counters with a stats jsonb column (queued/sent/delivered/bounced/failed/opened/clicked/unsubscribed), and the same worker's message_log usage relies on a body/channel/variables/scheduledAt shape that also does not match the canonical messageLog declaration (channelType/bodySnippet-only, no stored full body) - resolving this requires a genuine redesign of how the worker resolves and snapshots message content (likely: render body from campaigns.body + variables at send time, store only a bodySnippet for audit), not a mechanical import redirect. Disclosed and deferred per prove the disposition rather than mechanically canonicalize every historical module - tracked as a HIGH-priority, non-blocking-for-schema-truthing follow-up.",
    supportingCapability: ["lib/services/messaging/campaign-service.ts", "lib/workers/message-queue-processor.ts", "db/schema/domains/communications/campaigns.ts"],
    requiredRuntimePrivilege: 'FULL_DML',
    reviewPriority: 'HIGH',
  },
  {
  ...
  },
  {
    ...
    classification: 'TENANT_RLS_REQUIRED',
    reason: "3 conflicting declarations exist for physical table 'communication_preferences' (communication-analytics-schema.ts, domains/communications/analytics.ts, domains/communications/campaigns.ts \u2014 all 17-18 cols). NOTE: db/schema/phase-4-messaging-schema.ts's 'communicationPreferences' export is a DIFFERENT physical table ('communication_preferences_phase4') and does not participate in this conflict \u2014 lib/workers/message-queue-processor.ts's opt-in/opt-out check queries that separate table, not this one. Real consumers of THIS table: app/api/communications/unsubscribe/[recipientId]/route.ts, app/api/members/[id]/preferences/route.ts. Not yet individually verified against live schema in this pass (out of this round's bounded scope) \u2014 classification based on organization-scoped communications-preferences semantics consistent with its sibling tables; full canonicalization tracked as follow-up.",
    supportingCapability: ["app/api/communications/unsubscribe/[recipientId]/route.ts","app/api/members/[id]/preferences/route.ts"],
    requiredRuntimePrivilege: 'FULL_DML',
    reviewPriority: 'HIGH',
  },
  {
  ...
  },
  {
    table: 'message_log',
    classification: 'TENANT_RLS_REQUIRED',
    reason: "Verified 2026-09-01 against LIVE staging DB: 24 columns, organization_id present, matching db/schema/domains/communications/campaigns.ts's messageLog export. lib/services/messaging/campaign-service.ts already appears to use the canonical shape correctly; lib/workers/message-queue-processor.ts uses the stale db/schema/phase-4-messaging-schema.ts declaration (channel/body/variables/scheduledAt/externalId/nextRetryAt columns that do NOT physically exist \u2014 live has channel_type/body_snippet instead, no variables/scheduledAt/externalId/nextRetryAt at all). Same disclosed-and-deferred defect as 'campaigns' \u2014 see that entry for the full reasoning; both tables share the one broken worker.",
    supportingCapability: ["lib/services/messaging/campaign-service.ts", "lib/workers/message-queue-processor.ts", "db/schema/domains/communications/campaigns.ts"],
    requiredRuntimePrivilege: 'FULL_DML',
    reviewPriority: 'HIGH',
  },
