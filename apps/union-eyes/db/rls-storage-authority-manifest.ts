...
export const storageAuthorityManifest: StorageAuthorityEntry[] = [
  ...
  },
  {
    ...
    classification: 'TENANT_RLS_REQUIRED',
    reason: "3 conflicting declarations exist for physical table 'communication_preferences' (communication-analytics-schema.ts, domains/communications/analytics.ts, domains/communications/campaigns.ts \u2014 all 17-18 cols). NOTE: 'communication_preferences_phase4' is a DIFFERENT physical table, owned by the Django backend (backend/notifications/models.py) \u2014 its only TS-side mirror declaration (db/schema/phase-4-messaging-schema.ts) was deleted in round 4 as dead code (zero consumers; see 'campaigns' entry) and does not participate in this conflict. Real consumers of THIS table: app/api/communications/unsubscribe/[recipientId]/route.ts, app/api/members/[id]/preferences/route.ts. Not yet individually verified against live schema in this pass (out of this round's bounded scope) \u2014 classification based on organization-scoped communications-preferences semantics consistent with its sibling tables; full canonicalization tracked as follow-up.",
