...
$$ LANGUAGE plpgsql;
--> statement-breakpoint
-- PART 5a — member_documents tenant-column prerequisite (verbatim 0108). The
-- direct-org policy below scopes by organization_id, which this table lacks in
-- the source-native schema; add it + indexes, then the guarded no-op backfill.
...
--> statement-breakpoint
-- message_read_receipts references messages(id), not message_threads(id) — two-hop
-- override (verbatim 0108).
...
--> statement-breakpoint
-- PART 6b — grievance_deadlines: parent-owned via grievances (verbatim 0108).
...
--> statement-breakpoint
-- PART 7 — cross_org_access_log: system-only, fail-closed by omission (verbatim 0108).
