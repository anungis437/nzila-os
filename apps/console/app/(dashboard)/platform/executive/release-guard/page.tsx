/**
 * /platform/executive/release-guard — Release Guard agent surface.
 *
 * Loads open itsm_changes rows and runs the release-guard agent.
 */
...
function normalizeRisk(v: string): DbRiskLevel {
  return (['low', 'medium', 'high', 'critical'] as const).includes(v as DbRiskLevel)
    ? (v as DbRiskLevel)
    : 'medium'
}

async function loadSignal(orgId: string): Promise<ReleaseGuardSignal> {
  const now = new Date()
  ...
  const rows = await platformDb
    ...
    .where(
      and(
        eq(itsmChanges.orgId, orgId),
        notInArray(itsmChanges.status, ['closed', 'completed', 'rolled_back', 'failed']),
      ),
    )

  return {
    changes: rows.map((c) => {
      const approvers = Array.isArray(c.approverIds) ? (c.approverIds as unknown[]) : []
      const approved = Array.isArray(c.approvedByIds) ? (c.approvedByIds as unknown[]) : []
      const checklist = Array.isArray(c.implementationChecklist)
        ? (c.implementationChecklist as Array<{ done?: boolean }>)
        : []
      return {
        changeId: c.id,
        ...
        approversRequired: approvers.length,
        approversReceived: approved.length,
        hasRollbackPlan: Boolean(c.rollbackPlan && c.rollbackPlan.length > 0),
        checklistTotal: checklist.length,
        checklistDone: checklist.filter((c) => c.done === true).length,
        scheduledStart: c.scheduledStart ?? undefined,
...
async function lastInsights(orgId: string) {
  const [run] = await platformDb
    ...
    .where(and(eq(executiveAgentRuns.orgId, orgId), eq(executiveAgentRuns.agentKey, 'release-guard')))
...
export default async function ReleaseGuardPage() {
  ...
  async function run() {
    ...
    await runAndPersist(releaseGuardAgent, { orgId: o, actorId: u.id, triggeredBy: 'manual', input: signal })
    revalidatePath('/platform/executive/release-guard')
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <header className="mb-4">
        <h1 className="text-3xl font-semibold text-slate-900">Release Guard</h1>
        <p className="mt-2 text-sm text-slate-600">
          Change approvals, rollback plans, checklist completion, stale proposals.
          Live vs <code>itsm_changes</code>.
        </p>
      </header>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Release Guard agent</h2>
            <p className="text-xs text-slate-500">Ship fast without shipping unsafe.</p>
          </div>
          <form action={run}>
