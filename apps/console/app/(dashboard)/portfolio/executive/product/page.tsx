...
async function loadSignal(orgId: string): Promise<ProductStrategySignal> {
  const now = new Date()
  ...
  for (const r of rows) {
    if (!latestByProduct.has(r.product)) latestByProduct.set(r.product, r)
  }

  return {
    products: Array.from(latestByProduct.values()).map((r) => ({
      product: r.product,
      incidentsThisMonth: r.incidentsThisMonth,
      supportLoad: r.supportLoad,
      deploymentsShipped: r.deploymentsShipped,
      openBugs: r.openBugs,
...
async function lastInsights(orgId: string) {
  const [run] = await platformDb
    ...
    .where(and(eq(executiveAgentRuns.orgId, orgId), eq(executiveAgentRuns.agentKey, 'product-strategy')))
...
export default async function ProductStrategyPage() {
  ...
  async function run() {
    ...
    await runAndPersist(productStrategyAgent, { orgId: o, actorId: u.id, triggeredBy: 'manual', input: signal })
    revalidatePath('/portfolio/executive/product')
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <header className="mb-4">
        <h1 className="text-3xl font-semibold text-slate-900">Product Strategy</h1>
        <p className="mt-2 text-sm text-slate-600">Product health. Live vs <code>product_health_snapshots</code>.</p>
      </header>
      <nav className="mb-6 flex flex-wrap gap-2 text-sm">
        ...
        <Link href="/portfolio/executive/allocator" className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-slate-700 hover:bg-slate-50">Allocator</Link>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Product Strategy agent</h2>
            <p className="text-xs text-slate-500">Every product ships, runs, reports.</p>
          </div>
          <form action={run}>
