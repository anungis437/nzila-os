...
async function loadSignal(orgId: string): Promise<PmoSignal> {
  const now = new Date()
  ...
  return {
    initiatives: rows.map((r) => ({
      ...
      daysSinceUpdate: Math.max(0, Math.floor((now.getTime() - new Date(r.updatedAt).getTime()) / 86_400_000)),
    })),
    longRunningDays: 90,
  }
}
...
async function lastInsights(orgId: string) {
  const [run] = await platformDb
    ...
    .where(and(eq(executiveAgentRuns.orgId, orgId), eq(executiveAgentRuns.agentKey, 'pmo')))
...
export default async function PmoPage() {
  ...
  async function run() {
    ...
    await runAndPersist(pmoAgent, { orgId: o, actorId: u.id, triggeredBy: 'manual', input: signal })
    revalidatePath('/portfolio/executive/pmo')
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <header className="mb-4">
        <h1 className="text-3xl font-semibold text-slate-900">PMO</h1>
        <p className="mt-2 text-sm text-slate-600">Initiative discipline. Live vs <code>execution_initiatives</code>.</p>
      </header>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">PMO agent</h2>
            <p className="text-xs text-slate-500">Owner, due date, progress.</p>
          </div>
          <form action={run}>
