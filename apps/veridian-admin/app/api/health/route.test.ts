...
vi.mock('@nzila/os-core/health', () => ({
  ...
  healthStatusFromChecks: vi.fn(function(this: { isMocked?: boolean } | undefined, checks) {
    const isMocked = (this?.isMocked as boolean | undefined) ?? false
    if (isMocked) return 'degraded'
    return checks.process === 'ok' ? 'ok' : 'degraded'
  }),
