/**
 * Platform Admin — SAGE Phase 8B destruction approval API
 *
 * POST /api/sage/workspaces/[workspaceId]/destruction-requests/[requestId]/approve
 *
 * A DIFFERENT authorized human approves the exact frozen destruction scope. The
 * SAGE service recomputes every frozen value and returns CONFLICT on any drift.
 */
  ...
  return withOrgWrite(request, async (ctx) => {
    ...
    try {
      const result = await decideDestructionForScope(
        ctx,
        { workspaceId, requestId, decision: 'approved', rationale: parsed.data.rationale },
        { idempotencyKey },
      )
