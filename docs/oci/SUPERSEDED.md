...
| File | Disposition |
| --- | --- |
| `infotech/Nzila Ventures Business Plan — Revised (August 26, 2026).docx` | External, bank-submitted financing document. **Authoritative for pricing and named revenue** — this collapse pass made repo doctrine match it, not the reverse. Still uses legacy "ICRA" label in one line item (`Free Readiness Check (ICRA entry)`); that is a stale label inside the external document itself and is out of edit scope for this pass. |
