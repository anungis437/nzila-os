...
> **Status: `premature`.** Certification, inter-rater reliability, the assessor standard, the
> obligation taxonomy, the consequence model, explainability, policy traceability, and the
> "government-readiness master blueprint" below are internal engineering roadmap — not doctrine a
> buyer needs. Per [`OCI_METHOD.md` §9](../../OCI_METHOD.md#9-certification-pathway): certification
> exists only after two completed paid engagements and a second facilitator. None of that
> threshold has been met. See [`SUPERSEDED.md`](../../SUPERSEDED.md). Nothing in this directory is
> deleted; it is parked until the threshold is met, and it is not cited by the public method.
>
> **Blueprint — Architecture Review Only.** This program is documentation-only. No migrations,
> code, or tests are introduced by these documents. They specify a target architecture and a
