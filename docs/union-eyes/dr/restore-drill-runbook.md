# Union Eyes — DR Restore Drill Runbook
...
Define a repeatable, evidence-producing procedure for validating Union Eyes
disaster recovery capability. Each drill produces a structured evidence artifact
stored at `reports/dr/restore-drill-YYYY-MM-DD.json` and a human-readable
report at `reports/dr/restore-drill-YYYY-MM-DD.md`.

This runbook satisfies the reproducible restore-drill evidence requirement
defined in `apps/union-eyes/maturity.json` under `maturity_gaps.backup_restore`.
...
- [ ] Scratch database name chosen (default: `ue_drill_YYYYMMDD`)
...
- Procurement request for evidence within 30 days
...
psql -h $PGHOST -p $PGPORT -U $PGUSER -d ue_drill_YYYYMMDD \
  -c "SELECT schemaname, COUNT(*) FROM pg_tables GROUP BY schemaname;"

# Confirm hash-chain integrity on audit_events (if seeded)
psql -h $PGHOST -p $PGPORT -U $PGUSER -d ue_drill_YYYYMMDD \
  -c "SELECT COUNT(*), MAX(created_at) FROM audit_events;"

# App health check against staging (restore promoted)
...
psql -h $PGHOST -p $PGPORT -U $PGUSER -d postgres \
  -c "DROP DATABASE IF EXISTS ue_drill_$(date +%Y%m%d);"
...
- [Continuity Matrix](continuity-matrix.md)
