...
|---|---------|-------|-----:|---------:|--------|---------------|-------|
| 1 | `pnpm reality:anti-theatre` | repo | 0 | ~7 s | PASS_ARTIFACT_LEVEL | policy_gate | 0 errors, 1 249 warnings (R-6 × 307, R-7 × 936, R-8 × 5, R-2 × 1). R-3 is now enforced at artifact level: only files under `apps/union-eyes-demo/` may import demo/fixtures, and the rule covers static, dynamic (`await import(...)`), and CJS (`require(...)`) forms. Physical relocation of every demo file is complete; four operational files had their `isCupe4373DemoRuntime()` branches stripped. See `20_SEMANTIC_ISOLATION.md` for the full record and `16_ANTI_THEATRE_BASELINE.md` for the post-Stage-1 breakdown. Stage 2 (independent demo build, build-output demo-string scan) is not covered here. |
