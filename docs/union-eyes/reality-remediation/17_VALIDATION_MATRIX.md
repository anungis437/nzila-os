...
|---|---------|-------|-----:|---------:|--------|---------------|-------|
| 1 | `pnpm reality:anti-theatre` | repo | 0 | ~7 s | PASS_REGEX_ONLY | policy_gate | 0 errors, 1 262 warnings (R-6 × 318, R-7 × 938, R-8 × 5, R-2 × 1). R-3 was moved from 35 → 0 by (a) exempting `(demo)` route groups and (b) converting mixed-mode pages to dynamic imports outside the scanner's static-import pattern. **This is scanner cleanliness, not semantic demo isolation.** The demo modules are still physically present in `apps/union-eyes` and can be reached through runtime gates. Semantic isolation is Wave 0 §3 open work — see `16_ANTI_THEATRE_BASELINE.md` "SCANNER-CLEAN, NOT SEMANTICALLY ISOLATED" section and `19_AUTHORIZATION_VIOLATION.md` for the honest verdict correction. |
