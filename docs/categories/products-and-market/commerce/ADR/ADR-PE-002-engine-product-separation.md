...
3. **No formal distinction between an Engine and a Product.** The terms have been used interchangeably in plans, READMEs, and the governance manifest. There is no ADR defining what makes something an *engine* (headless, brand-neutral, SDK-only consumer surface) versus a *product* (branded, user-facing, opinionated UX with domain-specific workflows). Without that distinction, `pnpm platform:vs-app:check` cannot meaningfully enforce the rule it was built for.
...
- A two-layer model is the minimum cardinality that lets `pnpm platform:vs-app:check` produce a deterministic verdict: "is this code reused across products?" If yes → engine; if no → product. Three or more layers (engine / kit / product / framework / …) reintroduces the ambiguity we are removing.
...
- **`pnpm platform:vs-app:check` becomes decidable.** Reviewers get a deterministic verdict on every new module instead of a judgment call.
