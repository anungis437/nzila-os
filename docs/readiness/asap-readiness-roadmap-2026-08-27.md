...
4. Deploy and verify the Zonga readiness public-route fix only if Zonga is
   reactivated.
5. Bind Agrimo to an authoritative Django authority health endpoint only if
   Agrimo is reactivated.
6. Continue monitoring DORA deployment frequency as an operational KPI without
