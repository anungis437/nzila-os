...
- **Governance is structural, not aspirational.** 380+ contract tests enforce invariants at CI time: auth purity, revenue enforcement, control-plane authority, platform drift prevention. 30 GA gates — all passing.
...
| Contract enforcement | 380+ Vitest contract tests run on every PR via CI |
| 30 GA gates | Final deployment gates — all must pass before release |
...
> Flow (commerce) and CFO (finance) are also production-deployed revenue verticals. Full classification: [portfolio-matrix.md](portfolio-matrix.md).
