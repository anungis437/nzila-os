# CourtLens Refactor — Phase 0 Plan (Verified)

**Status:** `PHASE_0_READY_FOR_REVIEW`
**Planning branch:** `docs/courtlens-refactor-phase0`
**Planning branch head:** `6b6d3736dd3692112690165823a0702e1b6deea9`
**Baseline:** `main @ 6b6d3736dd3692112690165823a0702e1b6deea9` (identical — planning branch cut from main, contains zero product commits)
**Supersedes:** The prior in-tree draft classified `PHASE_0_DRAFTED_BUT_UNVERIFIED`.
...
Read from `apps/abr/**` at commit `6b6d3736dd3692112690165823a0702e1b6deea9`.
...
- **Emits no `logAuditEvent` calls.** (Confirmed by grepping `apps/abr/app/**` — audit calls exist only in `apps/abr/app/api/**/route.ts`.)

### 1.5 Read-audit doctrine — asymmetric, deliberately

- The **API route** `GET /api/courtlens/matters/[matterId]/route.ts` emits `action: 'courtlens.matter.viewed'` via `logAuditEvent`. Verified in the file at line ~55.
- The **server page** calls `getMatterDetail` from the module directly and does **not** emit an audit event.
...
### 1.11 Playwright / e2e — not on `main`

Verified from `apps/abr/package.json` on `main`:
...
- **No** `test:e2e` / `test:playwright*` script.
- **No** `apps/abr/e2e/**` directory on `main`.
- On `proof/courtlens-gap3-final`: `apps/abr/e2e/courtlens-review-packet.spec.mjs` and `apps/abr/playwright.config.ts` exist (added by commit `d7adb10ff`).

- **Correction (blocker #10):** The earlier draft referenced `apps/abr/tests/e2e/**` and `@axe-core/playwright`. That directory does not exist and axe-core is not in the workspace. Phase 1's test story must own Playwright installation as an explicit sub-task, not assume it.
...
- **Refactor branch** `refactor/courtlens-from-legacy` will branch from `main @ 6b6d3736`, **not** from `proof/courtlens-gap3-final`.
- **Inherited product commits:** none.
- **Excluded (evidence-only) commits:** `c7d5318bb`, `42f145b4b`, `f526f9c9b`.
- **Mixed commits, excluded from refactor branch:** `d7adb10ff`, `41e884a44`, `802f7834a`, `c5288b080` — reasons per-commit in the JSON.
- **Mixed commits, review-required:** `a622c8fbe` (vitest/playwright split) — recommend re-derivation rather than cherry-pick.
- **Candidate for independent PR against `main`:** `f2188d74b` (union-eyes / sage-core test-flakiness fixes). Not CourtLens; land separately if maintainers agree.

The refactor branch does **not** inherit `docs/courtlens/phase-2/**` or `docs/gap3/**` — those are Gap 3 evidence documents, not Phase 1 planning material.
...
| --- | --- |
| `EvidencePanel` / document upload | No `incident_documents` table exists on `main`. Needs schema + blob storage + antivirus. |
| `CommunicationPanel` / notes mutation | No note-write authorization contract exists for the refactor. |
| `ReviewerActions` extensions | Existing `ReviewerActions.tsx` already ships. |
...
- **Integration** (vitest, server): `page.test.tsx` extended with a case per role from §6.
- **E2E** (Playwright): **must install Playwright as part of Phase 1**, because `main` has no e2e infra. Do not cherry-pick the proof branch's `playwright.config.ts`; re-derive it. The `apps/abr/e2e/` directory is created for the first time in Phase 1.
...
Committed on `docs/courtlens-refactor-phase0`:
...
| Check | Result |
| --- | --- |
| `python -m pytest scripts/courtlens/tests -v` | 32 passed, 0 failed |
| Regenerate CSV to scratch, compare SHA-256 | Byte-identical (`C2F4…AF8D`) |
| `pnpm lint` | See `reports/phase0/lint.log` |
| `pnpm typecheck` | See `reports/phase0/typecheck.log` |
| `pnpm test:fast` | See `reports/phase0/test-fast.log` |
| `pnpm validate:docs` | See `reports/phase0/validate-docs.log` |
| `pnpm governance:audit` | See `reports/phase0/governance-audit.log` |
| Lefthook pre-commit | Ran with hooks enabled (no `LEFTHOOK=0`, no `--no-verify`) |

---

## 12. Open items for maintainer decision

1. **Approve the branch-base contract.** `refactor-branch-base.json` → `maintainerApproval.state`. Set to `approved` (or amend) before PR 1 opens.
2. **Land `f2188d74b` on main separately?** Union-eyes / sage-core test fixes are orthogonal to CourtLens and would improve `main` stability regardless. Recommend yes, as its own PR.
3. **Status-token strategy.** Either accept §7.1 (ABR-native tokens) or fund a preceding "wire `@nzila/ui/globals.css` into ABR" PR before Phase 1 opens.
4. **French translations.** Confirm the `[FR-DRAFT]` runtime banner approach in §8.2 is acceptable, or specify an alternative (parallel `fr-CA.draft.json` file, etc.).
5. **Playwright install** — accept that Phase 1 owns the first Playwright install in `apps/abr` on `main`.
