# CourtLens Refactor — Phase 0 Plan (Verified, v2)

**Status:** `PHASE_0_DRAFTED_BUT_UNVERIFIED`
**Planning branch:** `docs/courtlens-refactor-phase0-v2`
**Planning branch head:** self-referential; run `git rev-parse origin/docs/courtlens-refactor-phase0-v2` for the authoritative tip. See [reports/phase0/scope-report.json](../../reports/phase0/scope-report.json) for the machine-readable diff scope.
**Baseline (approved product line):** `integration/courtlens-gap3-product-line @ 1d0388c25c5813476aaf07b87f2c2a5564a59cff`
**Baseline lineage:** integration branch = `main @ 6b6d3736dd3692112690165823a0702e1b6deea9` + 6 CourtLens Gap-3 product commits (chronological); 3 evidence-only commits explicitly excluded. See [refactor-branch-base.json](refactor-branch-base.json) for the frozen contract and SHA map.
**Supersedes:** `docs/courtlens-refactor-phase0` (v1) — v1 was classified `PHASE_0_REQUIRES_REWORK` because it was based on pre-Gap-3 `main` and mixed unrelated commits (Windows lint-staged fix, union-eyes/sage test hardening). v1 remains on the remote as a forensic reference and MUST NOT be re-opened as a Phase 0 PR.
...
Read from `apps/abr/**` and `packages/**` at commit `1d0388c25c5813476aaf07b87f2c2a5564a59cff` (integration branch tip). Every claim below was re-verified against this baseline on 2026-07-19.
...
Full detail: [refactor-branch-base.json](refactor-branch-base.json).

Summary (v2):

- **Refactor branch** `refactor/courtlens-from-legacy` will branch from `integration/courtlens-gap3-product-line @ 1d0388c25` (which itself was rebuilt cleanly on `main @ 6b6d3736`).
- **Inherited product commits (6, chronological on the integration branch):**

  | Original SHA (proof branch) | Replayed SHA (integration branch) | Subject |
  | --- | --- | --- |
  | `d7adb10ff` | `0cc948a62` | feat(abr): close CourtLens Gap 3 implementation and proof tooling |
  | `41e884a44` | `bf1f40de8` | fix(abr): harden CourtLens review-packet audit persistence and proof coverage |
  | `a622c8fbe` | `353013d76` | fix(abr): separate vitest/playwright runners and stabilize audit org resolution |
  | `802f7834a` | `74bacc34a` | fix(abr,audit): persist canonical hash timestamp + version for independent recomputation (CourtLens Gap 3) |
  | `f2188d74b` | `2c8154e54` | fix(union-eyes,sage-core): repair pre-existing test failures uncovered during Gap 3 workspace validation |
  | `c5288b080` | `1d0388c25` | chore(db): regenerate schema snapshot for audit_events.occurred_at + hash_version |
...
- **Companion prerequisite branch** (independent of Phase 1, not merged into v2): `chore/windows-lint-staged-fix` — extracts the Windows-safe lint-staged fix onto `main`.
...
Committed on `docs/courtlens-refactor-phase0-v2`:
...
- [scripts/courtlens/build_legacy_inventory.py](../../scripts/courtlens/build_legacy_inventory.py)
...
| Check | Result |
| --- | --- |
| `python -m pytest scripts/courtlens/tests -v` | see [reports/phase0/pytest.log](../../reports/phase0/pytest.log) once regenerated on v2 |
| Regenerate CSV twice, compare SHA-256 | Byte-identical: `csvBodySha256 = 9668c0102676613bca8f43e166fc040506c34722466346cafcaa6b0cd590d261`, `legacyRootFingerprint = 56e42b0b568b59e40f1d5ddf701d70da0aa83ef3b139ac62227acd1284a29bb3` (rowCount = 380) |
| `pnpm validate:docs` | See [reports/phase0/validate-docs.log](../../reports/phase0/validate-docs.log) |
| `pnpm ownership:audit` | Regenerated on v2 — see [docs/ops/ownership-registry.md](../ops/ownership-registry.md) and [reports/ownership-registry.json](../../reports/ownership-registry.json) |
| `pnpm docs:index` | Regenerated on v2 — see [docs/documentation-index.md](../documentation-index.md) and [reports/documentation-index.json](../../reports/documentation-index.json) |
...
| `pnpm test:fast` | See [reports/phase0/test-fast.log](../../reports/phase0/test-fast.log) |
| Contract tests | Deferred to pre-push hook (see [reports/phase0/contract-tests.log](../../reports/phase0/contract-tests.log)) |
| Scope diff (v2 vs integration branch) | See [reports/phase0/scope-report.json](../../reports/phase0/scope-report.json) — Phase-0-only paths |
...
2. **Approve the CourtLens Phase 0 v2 review PR.** This branch (`docs/courtlens-refactor-phase0-v2`) supersedes v1. v1 must NOT be re-opened. v1 branch remains as a forensic reference and can be deleted only after v2 is merged.
3. **Sign off `docs/courtlens/legacy-inventory-review.json`.** All 380 entries are currently `reviewed=false`. Every entry destined for Phase 1 (`approvedPhase == 'phase-1'`) must have `reviewed=true`, `reviewer`, `reviewedAt`, `notes`, and `approvedDisposition`/`approvedTargetPath` filled before Phase 1 opens.
