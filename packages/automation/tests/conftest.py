...
_generators_core = _generators_root / "core"
_generators_migration = _generators_root / "migration"

for _p in (_automation_root, _generators_root, _generators_core, _generators_migration):
...
def mock_legacy_platform(temp_dir: Path):
    ...
    (platform_dir / "package.json").write_text(
        json.dumps(
            {
                "name": "mock-platform",
                "dependencies": {"@clerk/nextjs": "^5.0.0", "drizzle-orm": "^0.29.0"},
            }
        )
    )
...
def mock_manifest():
    ...
    return {
        ...
        "database": {"type": "postgresql", "provider": "azure-postgres"},
        "authentication": {"provider": "clerk", "configuration": {}},
    }
