...
class MigrationExecutor:
    ...
    def __init__(
        ...
        logger.info(f"Initialized MigrationExecutor (dry_run={dry_run})")

    def execute_migration(
        self, platform_id: str, manifest_path: Path, resume: bool = False
    ) -> MigrationResult:
        """Execute migration for a single platform"""

        with LogOperation(
            logger, "execute_migration", platform=platform_id, resume=resume
        ):
            ...
            try:
                ...
                phases = [
                    ...
                    (
                        ...
                        self._phase_provision_infrastructure,
                    ),
                ...
                for phase, phase_func in phases:
                    ...
                    phase_result = self._execute_phase_with_retry(
                        phase_func, platform_id, manifest_path, checkpoint
                    )
                    ...
                    if phase_result.get("success"):
                        ...
                        raise Exception(
                            f"Phase {phase.value} failed: {phase_result.get('error')}"
                        )
    ...
    def _execute_phase_with_retry(
        ...
        return {"success": False, "error": "Max retries exceeded"}  # pragma: no cover

    def _phase_analysis(
        self, platform_id: str, manifest_path: Path, checkpoint: MigrationCheckpoint
    ) -> Dict[str, Any]:
    ...
    def _create_checkpoint(
        self, platform_id: str, manifest_path: Path
    ) -> MigrationCheckpoint:
    ...
    def _load_checkpoint(self, platform_id: str) -> Optional[MigrationCheckpoint]:
        ...
        try:
            ...
            logger.info(
                f"Loaded checkpoint for {platform_id}: phase={data.get('phase')}"
            )
    ...
    def _save_checkpoint(self, checkpoint: MigrationCheckpoint):
        ...
        checkpoint_file = (
            self.checkpoint_dir / f"{checkpoint.platform_id}.checkpoint.json"
        )
    ...
    def rollback_migration(self, platform_id: str) -> bool:
        ...
        with LogOperation(logger, "rollback_migration", platform=platform_id):
            ...
            if not checkpoint:
                logger.warning(
                    f"No checkpoint found for {platform_id}, cannot rollback"
                )
                return False
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python migration_executor.py <platform-id> <manifest-path> [--resume] [--dry-run]"
        )
        sys.exit(1)
