...
class DjangoAPIGenerator:
    ...
    def load_analysis_data(self):
        ...
        print(
            f"✅ Loaded migration plan: {len(self.migration_plan.get('phase_1_critical', []))} critical services"
        )
        return True
    ...
    def generate_api_structure(self, service_info: Dict) -> Dict:
        ...
        if schema_imports:
            ...
            if "/" in schema_file:
                app_name = (
                    schema_file.split("/")[-1].replace("-schema", "").replace("_", "")
                )
            else:
...
"""
    ...
    def generate_migration_artifacts(self):
        ...
        for service_info in phase1_services:
            service_name = service_info["name"]
            print(
                f"   ⚙️  Generating API for: {service_name} ({service_info['operations']} ops)"
            )
            ...
            full_service = next(
                (
                    s
                    for s in self.service_analysis["services"]
