...
try:
    ...
    from logging_config import LogOperation, MigrationLogger
...
class PatternLibrary:
    ...
    def get_high_value_patterns(
        self, min_reusability: float = 0.7
    ) -> List[CodePattern]:
...
class PatternExtractor:
    ...
    def _extract_auth_patterns(self, platform_dir: Path, platform_id: str):
        ...
        for file_path in clerk_hook_files:
            if self._is_ignored(file_path) or "node_modules" in str(
                file_path
            ):  # pragma: no cover
                continue
    ...
    def _extract_database_patterns(self, platform_dir: Path, platform_id: str):
        ...
        db_files = list(platform_dir.glob("**/db.ts")) + list(
            platform_dir.glob("**/database.ts")
        )
        for file_path in db_files[:5]:
            ...
            try:
                ...
                logger.debug(
                    f"Error extracting database patterns from {file_path}: {e}"
                )

        # Django database utilities
        utils_files = list(platform_dir.glob("**/utils.py")) + list(
            platform_dir.glob("**/db_utils.py")
        )
    ...
    def _extract_api_client_patterns(self, platform_dir: Path, platform_id: str):
        ...
        # Fetch wrappers
        api_files = list(platform_dir.glob("**/api/**/*.ts")) + list(
            platform_dir.glob("**/services/**/*.ts")
        )
        for file_path in api_files[:10]:
            ...
            try:
                ...
                if re.search(
                    r"async\s+function\s+\w+\s*\([^)]*\)\s*{[^}]*fetch\(", content
                ):
                    pattern = self._create_pattern(
                        category="api-client",
    ...
    def _extract_utility_patterns(self, platform_dir: Path, platform_id: str):
        ...
        util_patterns = {
            ...
            "cn": (
                "Class Name Merger",
                "Tailwind class name merger (cn)",
                "TypeScript",
            ),
        }

        util_files = list(platform_dir.glob("**/utils/**/*.ts")) + list(
            platform_dir.glob("**/lib/**/*.ts")
        )
    ...
    def _calculate_reusability_scores(self, total_platforms: int):
        ...
        for pattern in self.patterns.values():
            ...
            pattern.reusability_score = (
                platform_score * 0.4 + occurrence_score * 0.3 + complexity_score * 0.3
            )
    ...
    def _build_library(self, total_platforms: int) -> PatternLibrary:
        ...
        reusable_occurrences = sum(
            p.occurrences for p in patterns_list if p.occurrences > 1
        )
        reuse_percentage = (
            (reusable_occurrences / total_occurrences * 100)
    ...
    def generate_report(self, library: PatternLibrary) -> str:
        ...
        high_value = sorted(
            library.patterns, key=lambda p: p.reusability_score, reverse=True
        )[:10]

        for i, pattern in enumerate(high_value, 1):
            report.append(f"{i}. {pattern.name} ({pattern.category})")
            report.append(f"   Reusability: {pattern.reusability_score:.2f}")
            report.append(
                f"   Platforms: {len(pattern.platforms)} ({', '.join(pattern.platforms[:3])}...)"
            )
...
def main():
    ...
    output_path = (
        Path(sys.argv[2]) if len(sys.argv) > 2 else Path("pattern_library.json")
    )
    ...
    platform_dirs = [
        d for d in legacy_root.iterdir() if d.is_dir() and not d.name.startswith(".")
    ]
