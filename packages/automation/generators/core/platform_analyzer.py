...
class PlatformAnalyzer:
    ...
    def analyze_platform(self, platform_path: Path) -> PlatformProfile:
        ...
        profile.components_count = self._count_components(
            platform_path, profile.tech_stack
        )
        profile.api_routes_count = self._count_api_routes(
            platform_path, profile.tech_stack
        )
        ...
        if stack.framework == "Django":
            ...
            for models_file in path.rglob("models.py"):
                if "migrations" not in str(models_file):
                    try:
                        ...
                        models_count += len(
                            re.findall(r"class\s+\w+\(.*models\.Model.*\):", content)
                        )
            ...
            if (path / "drizzle.config.ts").exists() or any(path.glob("**/drizzle/**")):
                ...
                for schema_file in path.rglob("**/schema.ts"):
                    try:
                        ...
                        db.models_count += len(
                            re.findall(r"export\s+const\s+\w+\s*=\s*pgTable", content)
                        )
                    except (IOError, UnicodeDecodeError):
                        pass
            ...
            # Check for Prisma
                ...
                try:
                    ...
                    db.models_count = len(
                        re.findall(r"^model\s+\w+", content, re.MULTILINE)
                    )
                    # Detect provider
                ...
                if migrations_dir.exists():
                    db.migrations_count = len(
                        [d for d in migrations_dir.iterdir() if d.is_dir()]
                    )
            ...
            # Check for Supabase
                ...
                for migration in (
                    migrations_dir.glob("*.sql") if migrations_dir.exists() else []
                ):
                    try:
                        ...
                        # Count RLS policies
                        db.rls_policies += len(
                            re.findall(r"CREATE POLICY", content, re.IGNORECASE)
                        )
                    except (IOError, UnicodeDecodeError):
                        pass
    ...
    def _count_api_routes(self, path: Path, stack: TechStack) -> int:
        ...
        if stack.framework == "Next.js":
            ...
            api_dir = (
                path / "app/api" if (path / "app").exists() else path / "pages/api"
            )
            if api_dir.exists():
            ...
            for route_file in path.rglob("*.ts"):
                if "routes" in str(route_file) or "api" in str(route_file):
                    try:
                        ...
                        count += len(
                            re.findall(r"\.(get|post|put|delete|patch)\(", content)
                        )
                    except (IOError, UnicodeDecodeError):
                        pass
    ...
    def _estimate_migration_time(self, profile: PlatformProfile) -> int:
        ...
        base_weeks = {"LOW": 2, "MEDIUM": 4, "HIGH": 8, "EXTREME": 12}
...
def main():
    ...
    output_path = (
        Path(sys.argv[2]) if len(sys.argv) > 2 else Path("platform_profiles.json")
    )
