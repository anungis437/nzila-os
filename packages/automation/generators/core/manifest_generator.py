...
class ManifestGenerator:
    ...
    def _infer_org_key(self, profile: Dict[str, Any]) -> str:
        """Infer org isolation key from business vertical"""
        ...
        for key, value in org_key_mapping.items():
            if key in vertical:
                return value
    ...
    def _select_modules(self, profile: Dict[str, Any], stack_profile: str) -> List[str]:
        ...
        if any(
            dep in ["openai", "langchain", "@anthropic-ai/sdk"] for dep in dependencies
        ):
        ...
        vertical = (profile.get("business_vertical") or "").lower()
        if any(
            kw in vertical for kw in ["banking", "fintech", "international", "diaspora"]
        ):
    ...
    def _infer_feature_flags(self, profile: Dict[str, Any]) -> Dict[str, bool]:
        ...
        flags = {
            "multi_org": True,  # Default to multi-org
    ...
    def generate_readme(self, manifests: Dict[str, Any], output_file: Path):
        ...
        for platform_id, manifest in sorted(manifests.items()):
            ...
            content.append(
                f"- **Framework**: {manifest['custom_config'].get('orm', 'N/A')}\n"
            )
            content.append(
                f"- **Migration**: {manifest['migration']['estimated_weeks']} weeks ({manifest['migration']['complexity']})\n"
            )
            content.append(
                f"- **Entities**: {manifest['migration']['entity_count']}\n\n"
            )
