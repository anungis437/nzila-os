...
# CourtLens — Deployment Guide

This document describes the OIDC-based deployment pipeline for
CourtLens targeting azure_container_apps (Azure Container Apps).
...
| `AZURE_ACR_IMAGE_REPO` | Full ACR image repository path (e.g. `courtlens-web`) |
...
2. The image is pushed to ACR at `courtlens-web:sha-<commit>`.
3. The Azure Container App revision is updated to use the new image.
4. Smoke tests run against `https://courtlens-staging.azurecontainerapps.io`.
...
2. The image is pushed to ACR at `courtlens-web:<tag>`.
3. The Azure Container App production revision is updated.
4. Health checks validate the deployment at `https://courtlens.azurecontainerapps.io`.
...
3. Verify the rollback at the appropriate URL (`https://courtlens-staging.azurecontainerapps.io` or `https://courtlens.azurecontainerapps.io`).
