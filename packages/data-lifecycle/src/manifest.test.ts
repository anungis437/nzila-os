...
import * as lifecycleIndex from './index'
...
describe('validateManifest', () => {
  ...
  it('detects missing required top-level fields and backup policy', () => {
    const bad = {
      appId: '',
      appName: '',
      version: '',
      ...
      residency: undefined,
      backup: undefined,
    } as unknown as DataLifecycleManifest
    ...
    expect(result.errors).toContain('Residency option is required')
    expect(result.errors).toContain('Backup posture is required')
  })

  it('detects non-positive backup RTO', () => {
    const bad: DataLifecycleManifest = {
      appId: 'test',
      appName: 'Test',
      version: '1.0.0',
      ...
      residency: { type: 'managed', regions: ['test'], orgSelectable: false, description: 'test' },
      backup: { frequency: 'daily', provider: 'test', location: 'test', encryptedAtRest: true, backupRetention: '7d', rtoHours: 0, rpoHours: 1 },
    }

    const result = validateManifest(bad)
    expect(result.valid).toBe(false)
    expect(result.errors).toContain('Backup RTO must be positive')
  })
})

describe('barrel exports', () => {
  it('exports manifest APIs from index', () => {
    expect(lifecycleIndex.generateAppManifest).toBe(generateAppManifest)
    expect(lifecycleIndex.generateAllManifests).toBe(generateAllManifests)
    expect(lifecycleIndex.validateManifest).toBe(validateManifest)
    expect(lifecycleIndex.APP_MANIFESTS).toBe(APP_MANIFESTS)
  })
})
