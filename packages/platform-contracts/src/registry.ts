...
const APP_REGISTRY_RAW: AppManifestInput[] = [
  ...
  },
  {
    id: 'trustcore',
    name: 'TrustCore',
    description: 'Trust and compliance engine for privacy governance, evidence trails, and Law 25 readiness',
    basePath: '/trustcore',
    tier: 'INCUBATING',
    appType: 'web-app',
    iconToken: 'shield-check',
    ...
    devPort: 3017,
    domains: ['trust', 'governance', 'compliance'],
    enabledCapabilities: ['auth', 'org-scope', 'telemetry', 'health-check', 'canonical-events'],
    governanceRequirements: [
      { controlId: 'TRC-01', name: 'Compliance evidence immutability', mandatory: true, evidenceClass: 'hash-chain', retentionClass: '7_YEARS' },
    ],
    integrationDependencies: [
      { provider: 'entra', required: true, type: 'auth', envVars: ['AUTH_SECRET', 'AZURE_AD_CLIENT_ID', 'AZURE_AD_TENANT_ID'] },
      { provider: 'postgresql', required: true, type: 'storage', envVars: ['DATABASE_URL'] },
    ...
    reportingBindings: {
      entityTypes: ['privacy-program', 'pia', 'incident', 'dsr', 'vendor'],
      eventTypes: ['trustcore.onboarding.completed', 'trustcore.incident.logged', 'trustcore.dsr.created'],
      metricNames: ['trustcore_compliance_score'],
      emitsFinancialRecords: false,
      reportingEnabled: true,
    },
    healthBinding: {
    ...
    deployment: {
      containerImage: 'nzila/trustcore',
      environments: ['local', 'staging'],
      requiresDatabase: true,
    },
    policyBindings: ['data-retention', 'access-control', 'compliance-evidence'],
    supportedOrgScopes: ['*'],
  },
]
