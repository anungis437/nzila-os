 ...
 *   - After max_retries attempts, message is marked failed and moved to dead-letter.
 *   - Manual intervention required to resolve (e.g., fix encryption key, update email).
 */

import type { SageDeliveryNotifier } from './delivery-notifier'
import type { SageRepository } from './repository'
import type { SageSqlClient } from './sql-client'
import { decryptNotificationPayload } from './notification-encryption'
...
export class NotificationDispatcher {
  ...
  private async dispatchOne(
    notification: any, // SageNotificationOutbox
  ): Promise<{ success: boolean; error?: string }> {
    try {
      // Decrypt the payload with AAD verification (binds to this specific grant)
      const aad = `${notification.grantId}:${notification.messageId}`
      const payload = decryptNotificationPayload(notification.encryptedPayload, aad)
      if (!payload) {
      ...
      try {
        ...
        await this.repo.markNotificationDispatched({
          id: notification.id,
          dispatchOwner: this.config.dispatcherInstanceId,
          providerMessageId: notification.id, // Provider should return an ID; stubbed here
        })
        ...
        // Update nextAttemptAt and release the lease
        await this.sql.query(
          ...
          [notification.id, this.config.dispatcherInstanceId],
        )
  ...
  async recoverStaleLeases(): Promise<{ recovered: number }> {
    const staleLeases = await this.sql.query(
    ...
    for (const row of staleLeases.rows) {
      const res = await this.repo.releaseNotificationOutboxToPending((row as any).id)
      if ((res as any).success) recovered++
    }
