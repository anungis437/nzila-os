...
describe('Phase 8A.1: Crash-Safe Invitation Delivery', () => {
  ...
  describe('Message-ID Stability', () => {
    it('should generate stable messageId from grant ID (not timestamp)', () => {
      const grantId = 'grant-stable-123'

      // Simulate two different timestamps
      const messageId1 = `sage-delivery-invitation:${grantId}`
      const messageId2 = `sage-delivery-invitation:${grantId}`

      // THEN: messageIds are identical (not timestamp-dependent)
      expect(messageId1).toBe(messageId2)
      expect(messageId1).toMatch(/^sage-delivery-invitation:grant-/)
    })
  })
  ...
  describe('Lease and Fence Pattern', () => {
    it('should prevent concurrent claims with lease', async () => {
      ...
      await repo.enqueueNotificationOutbox({
        intent: {
  ...
  describe('Error Scenarios', () => {
    it('should mark notification as dispatched on success', async () => {
      ...
      expect(updated?.encryptedPayload).toBe('')
      expect(updated?.payloadDestroyedAt).toBeTruthy()
    })

    it('should dead-letter a terminal notification error', async () => {
      await repo.enqueueNotificationOutbox({
        intent: {
          messageId: 'msg-fail',
      ...
      const result = await repo.markNotificationDeadLetter({
        id: claimed!.id,
        dispatchOwner: 'test-dispatcher',
