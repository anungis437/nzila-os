    ...
    const { rows } = await this.sql.query<SageDeliveryGrantRow>(
      `with issued_req as (
       ...
       notification as (
         ...
         select $22, $1, $2, $3, $5, $23, $24, $25, $26, 'sage-notification:v1',
                'pending', 0, 5, $11
         from issued_req
         returning id, delivery_request_id, provider, template, recipient_id
       ),
       grant_row as (
         ...
         select $1, $2, notification.delivery_request_id, $4, notification.recipient_id,
  ...
  async markNotificationDispatched(input: {
    ...
    const { rows } = await this.sql.query<{ id: string }>(
      `update sage_notification_outbox
       ...
       where id = $1 and dispatch_owner = $4
       returning id`,
