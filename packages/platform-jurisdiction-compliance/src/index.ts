...
export {
  ...
  calculateTax,
  validateCorporateTaxRate,
  validateCooperativeRegistration,
  validateAddress,
  validateExamType,
  validateExamGrade,
  validateCertificateFreshness,
  validateEmployeeRecord,
  validateCooperativeRecord,
} from './validators'
export type {
  ValidationResult,
  EmployeeRecord,
...
export {
  generateKenyaCooperative,
  generateKenyaFarmers,
  generateKenyaExaminee,
  generateUgandaCooperative,
  generateUgandaFarmers,
  generateUgandaExaminee,
  generateNigeriaCooperative,
  generateNigeriaFarmers,
  generateNigeriaExaminee,
  generateTestDataset,
} from './test-datasets'
export type {
  TestCooperative,
...
export const jurisdictions: Record<JurisdictionCode, JurisdictionMetadata> = {
  KE: {
    code: 'KE',
    displayName: 'Kenya',
    region: 'East Africa',
 ...
 */
export function getAgrimoTaxPolicy(code: JurisdictionCode) {
 ...
 */
export function getAgrimoPensionPolicy(code: JurisdictionCode) {
 ...
 */
export function getNACPExamPolicy(code: JurisdictionCode) {
 ...
 */
export function getFeatureFlags(code: JurisdictionCode) {
 ...
 */
export function isJurisdictionLaunchReady(
  ...
  if (jurisdiction === 'KE' && app === 'agrimo') {
    ...
    if (!getAgrimoTaxPolicy('KE').supportedCrops) blockers.push('Missing crop list')
  }

  if (jurisdiction === 'NG' && app === 'nacp') {
    // Nigeria + NACP: require NBTE compliance
    const policy = getNACPExamPolicy('NG')
    if (policy.maxAttempts < 2) blockers.push('Insufficient exam attempts allowed')
  }
...
export interface LaunchChecklist {
  jurisdiction: JurisdictionCode
  app: string
  items: { label: string; completed: boolean; notes?: string }[]
}
 ...
 */
export function getJurisdictionName(code: JurisdictionCode): string {
 ...
 */
export function getJurisdictionInfo(code: JurisdictionCode) {
 ...
 */
export function isSupportedJurisdictionCode(code: string): code is JurisdictionCode {
