 ...
 *
 * Verifies:
 *   1. ALL Next.js app proxy files import the rate limiter from os-core
 *   2. /api/health is in the isPublicRoute allowlist of all proxies
 ...
 *   4. Rate limit env vars are referenced in proxy (RATE_LIMIT_MAX / RATE_LIMIT_WINDOW_MS)
 *
...
function hasHealthInPublicRoutes(content: string): boolean {
  // Should have /api/health in the public route matcher (isPublicRoute, isPublicPath, publicPaths,
  // or a direct startsWith check that exempts the health endpoint from rate limiting)
  ...
  return /isPublicRoute.*\/api\/health/.test(collapsed) ||
    ...
    /startsWith\(['"]\s*\/api\/health/.test(content)
}
...
describe('Rate Limiting — REM-01 contract', () => {
  ...
  for (const app of RATE_LIMIT_APPS) {
    const mwPath = `apps/${app}/proxy.ts`

    it(`${app} proxy imports rate limiter`, () => {
      const content = read(mwPath)
      expect(content).not.toBe('')
      expect(
        importsRateLimiter(content),
        `apps/${app}/proxy.ts must import checkRateLimit or rateLimitHeaders from @nzila/os-core/rateLimit`,
      ).toBe(true)
    })

    it(`${app} proxy enforces a rate-limit max cap`, () => {
      const content = read(mwPath)
      expect(content.includes('RATE_LIMIT_MAX') || content.includes('max:')).toBe(true)
    })

    it(`/api/health is in ${app} proxy public-route allowlist`, () => {
      const content = read(mwPath)
