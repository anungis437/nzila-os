/**
 * Tests for the flow + zonga per-app seeders.
 *
 * Both seeders follow the same plan-only Phase 2 contract as union-eyes:
...
// Side-effect imports: register both seeders.
import * as flowSeeder from '../src/seeders/flow'
import * as zongaSeeder from '../src/seeders/zonga'

interface SeederBundle {
  readonly app: SeedApp
  readonly stagingOrgId: string
  readonly seederMod: { seeder: import('../src/index').SeederModule }
}

const BUNDLES: readonly SeederBundle[] = [
  {
    app: 'flow',
    stagingOrgId: 'org-flow-staging-merchant-9999',
    seederMod: { seeder: flowSeeder.seeder },
  },
  {
    app: 'zonga',
    stagingOrgId: 'org-zonga-staging-label-9999',
    seederMod: { seeder: zongaSeeder.seeder },
  },
]
...
for (const bundle of BUNDLES) {
  describe(`${bundle.app} seeder`, () => {
    ...
    it(`exposes the well-known staging org "${bundle.stagingOrgId}"`, () => {
      const mod = bundle.app === 'flow' ? flowSeeder : zongaSeeder
      expect(mod.STAGING_ORG.id).toBe(bundle.stagingOrgId)
      expect(mod.STAGING_ORG.name).toMatch(/staging/i)
    })
