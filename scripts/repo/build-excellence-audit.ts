...
function main(): void {
  ...
  const release = loadJson<unknown>(root, 'reports/release-governance-audit.json')
  const sre = loadJson<unknown>(root, 'reports/sre-executive-dashboard.json')
  const finops = loadJson<unknown>(root, 'reports/finops-summary.json')
  const docsIndex = loadJson<unknown>(root, 'reports/documentation-index.json')
  const ownership = loadJson<unknown>(root, 'reports/ownership-registry.json')
  const dora = loadJson<unknown>(root, 'ops/outputs/dora-metrics.json')
